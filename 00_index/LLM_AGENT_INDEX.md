...
status: active
created: 2026-06-30
scope: AI agent navigation and retrieval policy
---
...
| Employment | [[50_취업 준비/00_취업준비_목차]] | 취업 준비·지원 자료 MOC | 프로젝트 경험의 증거화, 사례화, 문항 매핑과 현재 지원서 작업으로 라우팅한다. |
| Personal | [[77_개인/00_개인_목차]] | 개인 장기 기록 MOC | 게임·생활 등 다른 업무 영역과 분리된 개인 기록으로 라우팅한다. |
...
| AWS 공식 강사 / Architecting on AWS 수업 | [[10_학습 노트/클라우드/AWS/00_AWS_목차]] | [[10_학습 노트/클라우드/공식 Arc 과정/raw 메모]], [[40_자료/강의 자료/AWS Arc/README|Architecting on AWS 원본 PDF 목차]], 필요한 모듈 PDF | 기존 `AWS기초.pdf` 흐름과 섞거나 미승인 Source Digest를 현재 지식으로 사용하지 말 것 |
...
| 3차 퍼블릭 클라우드 보안 운영 프로젝트 | [[20_팀 프로젝트/3차 프로젝트/00_3차프로젝트_목차]] | [[20_팀 프로젝트/3차 프로젝트/와주/00_Wazuh MOC|Wazuh MOC]], 5장 보안관제 보고서, As-built/Runtime 검증, 시연 근거 | 2026-07-28 초기 계획·후보 snapshot을 최종 상태로 보거나 파일 존재만으로 E2E Runtime 성공을 주장하지 말 것 |
| 프로젝트 경험을 자소서·면접 사례로 재사용 | [[50_취업 준비/취업 코칭 작업실/00_취업 코칭 MOC]] | `01_증거 장부 → 02_실제 사례 후보 → 03_자소서 문항 매핑`; 현재 지원서 상태는 `09`, 3차 프로젝트 준비 정리는 `10` | 지원서 문장 자체를 사실 근거로 사용하거나 팀 결과를 개인 기여로 변환하지 말 것 |
| 메이플 우노03 장비·성장 계획 | [[77_개인/메이플/00_메이플_목차]] | 장비 강화 계획, [[scripts/maple/README|메이플 캐릭터 정보 수집기]], 필요 시 최신 AI 컨텍스트 | 오래된 계획 수치나 이전 JSON을 현재 상태로 단정하지 말 것 |
...
|---|---|---|
| AWS class split | `Home.md`, AWS MOC, 공식 Arc RAW, 원본 PDF 목차 | `AWS기초.pdf` 흐름은 완료된 별도 학원 과정으로 보고, 공식 Arc 현재진행 RAW·원본 PDF와 섞지 않는다. |
| Old AWS material | AWS MOC의 `AWS기초.pdf` 기준 시점 경고 | Console UI, 비용, Free Tier, 보안 기본값은 현재 기준으로 재확인한다. |
| 3차 프로젝트 초기 snapshot | 3차 프로젝트 MOC의 `초기 계획·후보 자료`, Wazuh MOC의 현행 Rule 지도 | 7월 후보·인수인계·첫날 일일 로그는 결정 과정의 과거 기록이다. 최종 구현·Runtime 기준으로 승격하지 않는다. |
| 3차 프로젝트 Runtime claim | As-built/Runtime 검증, 시연 장면별 근거, 현재 Rule/Workflow | Source 수집, Rule Alert, Shuffle Execution, GitHub Run, Argo 적용, Pod 상태를 서로 다른 단계로 보고 식별자가 연결된 범위만 E2E로 주장한다. |
...
| AWS 공식 강사 / ARC 안정 학습 노트 미작성 | AWS MOC에서 현재 RAW와 원본 PDF 목차로는 연결되지만, 승인된 안정 학습 노트는 아직 없음 | 안정 노트가 실제로 만들어질 때만 재시작 지점을 추가하고, 그 전에는 RAW와 PDF를 구분해 사용 |
