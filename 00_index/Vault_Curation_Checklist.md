...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `CSRF` 범위를 점검한다. PDF p.98-106을 기준으로, 기존 [[10_학습 노트/시스템보안/웹보안/CSRF|CSRF]]와 [[10_학습 노트/시스템보안/웹보안/CSRF를 이용한 회원정보 변경 실습|CSRF를 이용한 회원정보 변경 실습]]이 concept/lab으로 충분히 분리되어 있는지 먼저 확인한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: CSRF, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
