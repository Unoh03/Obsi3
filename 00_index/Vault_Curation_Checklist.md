...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `로그인 Brute Force와 계정 보호` 범위를 점검한다. PDF p.77-79를 기준으로, 기존 [[10_학습 노트/시스템보안/웹보안/Hydra 로그인 Brute Force 실습|Hydra 로그인 Brute Force 실습]]이 방어 관점까지 충분히 흡수했는지 먼저 확인한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: 로그인 Brute Force와 계정 보호, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
