...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `기초 웹 인증 방식 - Basic, Anonymous, Form`을 stable concept note 단위로 만들지 판단한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: 기초 웹 인증 방식, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
