...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `Web Session Hijacking` 범위를 점검한다. PDF p.80-82를 기준으로, 기존 [[10_학습 노트/시스템보안/웹보안/Web Session Hijacking|Web Session Hijacking]]이 세션/쿠키, XSS, Sniffing 경로와 충분히 연결되어 있는지 먼저 확인한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: Web Session Hijacking, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
