...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `XSS` 범위를 점검한다. PDF p.83-97을 기준으로, 기존 [[10_학습 노트/시스템보안/웹보안/XSS|XSS]], [[10_학습 노트/시스템보안/웹보안/XSS를 이용한 Session Token 탈취 실습|XSS를 이용한 Session Token 탈취 실습]], [[10_학습 노트/시스템보안/웹보안/실무형 XSS 방어|실무형 XSS 방어]], [[10_학습 노트/시스템보안/웹보안/HTML 인코딩|HTML 인코딩]]이 concept/lab/defense로 충분히 분리되어 있는지 먼저 확인한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: XSS, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
