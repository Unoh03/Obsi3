...
- [ ] 웹보안 stable / RAW / PDF source 경계 점검
  ...
  - Next check: `SQL Injection 방어` 범위를 점검한다. PDF p.132를 기준으로, 기존 [[10_학습 노트/시스템보안/웹보안/SQL Injection 방어|SQL Injection 방어]]가 SQL Injection 흐름을 닫는 방어 stable concept note로 충분한지 먼저 확인한다.
...
- [ ] 시스템보안 전체 재시작 지점 점검
  ...
  - Next check: SQL Injection 방어, DHCP Snooping, IP Source Guard, Wireshark 필터를 실제 후보 노트 단위로 쪼갤지 판단한다.
