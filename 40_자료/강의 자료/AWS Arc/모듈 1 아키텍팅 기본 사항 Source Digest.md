...
| 위치 | 항목 | 조치 |
|---|---|---|
| p.47 / 파일 p.21 | `비용 최적화` 예시 문구 중 “지출 분석 및 귀하”로 보이는 부분 | 원본 PDF 확대 또는 강의 자료 재확인 필요 |
| p.62 / 파일 p.36 | `신뢰성`과 `안정성` 번역 혼용 | 같은 Well-Architected `Reliability` pillar를 가리키는 표현으로 메모 유지 |
| 전체 | 중간 블록별 Coverage Check 제거 | 최종 Coverage Map으로 대체 완료 |
| 전체 | 코드펜스 닫힘 문제 | ` ``` ` 형태로 정규화 완료 |
...
- 다음 단계: PDF 원문 대조 검수 후 Concept Note 분리 여부 판단
