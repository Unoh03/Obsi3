...
- Quiz의 문제·보기·정답·해설
- 도식·Screenshot·색상·범례가 전달하는 의미와 연결 관계
- Page 순서와 원자료 위치
...
5. 원자료 내용과 Visual 의미를 Chapter Digest에 기록한다.
6. Code·명령어·설정은 기호·공백·들여쓰기를 원본과 대조한다.
7. Coverage에서 모든 Page의 처리 상태를 기록한다.
8. 판독 불가·충돌·현재성 문제는 추측으로 메우지 않고 검토 대기로 남긴다.
...
- [ ] 정보성 Page를 Rendering하여 Visual 의미를 검수했다.
...
Navigation 검사는 MOC·Index route를 수정했을 때 실행한다. PDF 내용의 무손실 여부는 Markdown Link 검사만으로 증명할 수 없으므로 Page Coverage와 원본 대조 결과를 별도로 확인해야 한다.
