...
flowchart TB
    subgraph REQUEST["외부 요청 처리 경로"]
        User["사용자·실습자"] --> Route53["Amazon Route 53<br/>DNS 해석"]
    ...
    ALB -.-> ALBLog
    DVWA -.-> AppLog
    K8sAPI["kubectl·Controller의 Kubernetes API 호출"] --> EKSLog
    AWSAPI["Console·CLI·Terraform·AWS Service의 API 호출"] --> CTLog
    Network["VPC ENI의 거부 Traffic"] --> FlowLog
