...
| **ALB Vended Access/Connection/Health Logs** | **공식 정성 표현:** CloudWatch Logs에서 Live Tail로 실시간 관찰 가능. 이벤트별 정량 latency는 미공개 | 현재 미사용 | ALB Vended Logs → CloudWatch Logs → Subscription → Lambda → SQS → Bridge → Wazuh | **저지연 SIEM 수집의 우선 평가 후보.** Legacy보다 실제 얼마나 빠른지는 latency·누락·중복·비용 Runtime 비교 전 확정 금지. 필요 시 S3 Archive 병행 | Target Candidate / Runtime 비교 필요 |
