...
| **ALB Legacy Access Logs → S3** | **공식 수치:** 노드별 5분마다 Log file 게시 + eventual consistency | ALB → S3 → Wazuh S3 List/Get 10분 Poll | 같은 Legacy를 유지한다면 S3 `ObjectCreated` → SQS → Bridge → Wazuh | Legacy를 주 탐지 Source로 유지하지 말고 **ALB Vended Logs를 주 SIEM Source로 전환**, Legacy/S3는 Archive 역할 | `ALB=5분 고정`은 Legacy에만 해당 |
| **ALB Vended Access/Connection/Health Logs** | **공식 정성 표현:** CloudWatch Logs에서 Live Tail로 실시간 관찰 가능. 이벤트별 정량 latency는 미공개 | 현재 미사용 | ALB Vended Logs → CloudWatch Logs → Subscription → Lambda → SQS → Bridge → Wazuh | **현재 기준 ALB의 권장 SIEM Route.** CWL에서 Event-driven으로 빼고 필요 시 S3 Archive 병행 | Legacy보다 우선 검토 |
| **CloudFront Standard Logs (v2/legacy)** | **공식 수치:** 일반적으로 이벤트 후 1시간 이내, 일부 항목은 최대 24시간 지연 가능 | 현재 CloudFront CloudWatch Logs → Wazuh `GetLogEvents` Poll 계열 | 같은 Standard를 유지하면 CWL/Firehose 직접 전달 후 Event-driven 소비 가능하지만 **Standard 자체 생성 지연은 남음** | 실시간 Trigger로는 부적합. **Evidence/Archive** 용도로 유지 | Destination을 바꿔도 Standard의 Source latency는 사라지지 않음 |
...
→ 하지만 CloudTrail이라는 Audit Source 자체의 평균 약 5분 지연은 남음
```
...
| **NLB Vended Access Logs** | **공식 수치 미공개.** CWL/Firehose/S3 직접 전달 지원 | NLB Vended → CWL → Subscription → SIEM | Legacy 5분 파일보다 우선 검토할 신규 경로 |
