...
| ---------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------- | --------------------------------------------------- |
| A. Source 생성·집계 지연           | 실제 Event가 발생한 뒤 해당 서비스가 소비 가능한 Log/Event Record를 만들어내기까지의 시간                         | ALB Access Log 생성, VPC Flow Log aggregation              | 대부분 직접 줄이기 어려움. 더 빠른 로그 종류가 있다면 Source 자체를 바꿀 수는 있음 |
| B. Source-native Delivery 지연 | 생성된 Record가 CloudWatch Logs, S3, EventBridge, Kinesis 등의 소비 가능한 AWS 목적지에 나타날 때까지의 시간 | CloudTrail → CloudWatch Logs, ALB → S3                   | 서비스가 지원하는 더 빠른 Delivery 방식으로 바꿀 수 있는 경우가 있음         |
| C. SIEM Transport 지연         | 소비 가능한 Event를 실제 Wazuh까지 전달해 Rule 평가가 시작되기까지의 시간                                     | 10분 Poll 또는 Subscription → Lambda → SQS → Bridge → Wazuh | 프로젝트에서 가장 직접적으로 줄일 수 있는 구간                          |
...
| **AWS WAF Web ACL Logs** | **공식 수치 미공개.** 요청별 로그를 CloudWatch Logs/S3/Firehose로 전송 가능 | WAF → CloudWatch Logs → Wazuh `GetLogEvents` 10분 Poll | WAF → CloudWatch Logs → Subscription → Lambda → SQS → Bridge → Wazuh | **CWL Subscription 기반 Event-driven 전환.** 장기 보존이 필요하면 별도 Archive 유지 | Poll 대기 제거 효과 큼 |
| **ALB Legacy Access Logs → S3** | **공식 수치:** 노드별 5분마다 Log file 게시 + eventual consistency | ALB → S3 → Wazuh S3 List/Get 10분 Poll | 같은 Legacy를 유지한다면 S3 `ObjectCreated` → SQS → Bridge → Wazuh | Legacy/S3는 Archive·Replay 역할로 유지하고, **ALB Vended Logs는 저지연 SIEM 수집을 위해 우선 평가할 후보**로 둠. 실제 우월성은 Runtime 비교 전 확정하지 않음 | `ALB=5분 고정`은 Legacy에만 해당 |
| **ALB Vended Access/Connection/Health Logs** | **공식 정성 표현:** CloudWatch Logs에서 Live Tail로 실시간 관찰 가능. 이벤트별 정량 latency는 미공개 | 현재 미사용 | ALB Vended Logs → CloudWatch Logs → Subscription → Lambda → SQS → Bridge → Wazuh | **저지연 SIEM 수집의 우선 평가 후보.** Legacy보다 실제 얼마나 빠른지는 latency·누락·중복·비용 Runtime 비교 전 확정 금지. 필요 시 S3 Archive 병행 | Legacy보다 우선 검토 |
| **CloudFront Standard Logs (v2/legacy)** | **공식 수치:** 일반적으로 이벤트 후 1시간 이내, 일부 항목은 최대 24시간 지연 가능 | 현재 CloudFront CloudWatch Logs → Wazuh `GetLogEvents` Poll 계열 | 같은 Standard를 유지하면 CWL/Firehose 직접 전달 후 Event-driven 소비 가능하지만 **Standard 자체 생성 지연은 남음** | 실시간 Trigger로는 부적합. **Evidence/Archive** 용도로 유지 | Destination을 바꿔도 Standard의 Telemetry 사용 가능 지연(A+B)은 사라지지 않음 |
| **CloudFront Real-time Access Logs** | **공식 수치:** 요청 수신 후 수초 내 Kinesis Data Streams 전달. Best-effort이며 드물게 지연/누락 가능 | Native Real-time Logs는 현재 주 경로가 아님 | CloudFront Real-time Logs → Kinesis → Consumer/Lambda → SQS → Bridge → Wazuh | **CloudFront 고유 필드가 저지연 탐지에 필요할 때 채택.** 그렇지 않으면 WAF를 빠른 Edge 보안 신호로 쓰고 Standard CloudFront Logs는 Evidence로 두는 편이 비용·중복 측면에서 합리적 | 조건부 최적 |
| **CloudTrail S3 Data Event (`GetObject`)** | **공식 수치:** API 호출 후 CloudWatch Logs/S3에 평균 약 5분, 보장값 아님 | CloudTrail → Security Log S3 → Wazuh S3 List/Get 10분 Poll | CloudTrail → CloudWatch Logs → Subscription → Lambda → SQS → Bridge → Wazuh | **CWL Event-driven을 SIEM Route로 사용하고 Trail S3는 Audit/Archive로 유지.** CloudTrail Trail 경로의 Telemetry 사용 가능 지연(A+B) 평균 약 5분은 제거할 수 없음 | `GetObject` Confirmation에는 여전히 유효 |
...
CloudTrail
→ Transport는 Event-driven으로 개선 가능
→ 하지만 CloudTrail Trail 경로에서 API Event가 CloudWatch Logs 또는 S3에 사용 가능해지기까지의 평균 약 5분 지연은 남음
```
...
| **NLB Vended Access Logs** | **공식 수치 미공개.** CWL/Firehose/S3 직접 전달 지원 | NLB Vended → CWL → Subscription → SIEM | 저지연 SIEM 수집의 우선 평가 후보. Legacy보다 빠르다고 확정하지 않으며 Runtime 비교 필요 |
