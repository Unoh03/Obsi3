...
2. Wazuh Dashboard 상단의 `100102` `100110`, `100111`를 보여준다.
	1. `100102`: 공격을 탐지하는 Rule이 아니다. DVWA 애플리케이션이 생성한 안전한 시험용 보안 로그가 전송 과정에서 유실되지 않고 Wazuh까지 도착했으며, Wazuh가 해당 로그를 정상적으로 해석했는지 확인하는 상태 점검용 Rule이다.
	   
	2. `100110`: 취약한 웹 애플리케이션에서 실행된 명령이 AWS EC2 인스턴스의 권한 정보가 제공되는 Metadata Credential endpoint에 접근하고, 그 요청의 출력까지 반환된 상황을 조기에 탐지한다. 아직 S3 데이터 유출 자체를 확정한 것은 아니지만, 공격자가 클라우드 권한을 탈취하려는 단계까지 접근했다는 강한 위험 신호다.
	   
	3. `100111`: AWS의 감사 로그인 CloudTrail을 분석해, 탈취가 의심되는 권한으로 보호 대상 S3 객체를 요청했고 HTTP 200 응답과 함께 실제 데이터 바이트가 반환된 사실을 확인한다. 따라서 이 Rule은 권한 접근 시도를 넘어 실제 보호 데이터가 반환된 침해확정 단계로 사용한다. 촬영을 시작하기 전에 세 Rule의 현재 Count를 기록한다. 이후 공격 과정에서 `100110`과 `100111`이 각각 언제 증가하는지 비교해, 초기 침투 징후가 실제 데이터 접근으로 발전하는 흐름을 시간순으로 확인한다.
