...
**예상 길이:** 50~70초
**화면:** DVWA `security.php` → Wazuh Dashboard → 와주 룰 파일 → Argo CD

### 화면 큐와 조작

1. DVWA의 `Security level is currently: low`를 3초간 보여준다.
2. Wazuh Dashboard 상단의 Rule `100102`, `100110`, `100111` Count를 차례로 보여주고 시작값을 기록한다.
3. Argo CD의 `Synced / Healthy`와 정상 DVWA Pod를 보여준다.

### 그대로 읽을 대사

> 지금부터 교육용 은행 애플리케이션을 대상으로 한 공격과 SOC 자동 대응을 시연하겠습니다.

> 현재 DVWA는 공격을 재현할 수 있는 low 상태이고, Argo CD에는 이전 공격에서 격리된 Pod가
> 남아 있지 않습니다.

> Wazuh 상단에는 세 가지 지표가 있습니다. Rule 100102는 공격 경고가 아니라 DVWA 보안 로그가 Wazuh에 정상적으로 수집되고 있는지 확인하는 상태 지표입니다.
> 와주가 현재 개인 노트북에서 컨테이너로 운용중이라 

> Rule 100110은 공격 명령이 EC2의 Credential endpoint에 접근해 출력을 반환한 시점을 조기에
> 탐지합니다. 아직 데이터 유출을 확정한 것은 아니지만, 클라우드 권한 탈취 직전의 고위험
> 신호입니다.

> Rule 100111은 해당 권한으로 보호된 S3 객체를 요청해 실제 데이터가 반환된 사실을 확인하는
> 침해확정 Rule입니다.

> 지금 세 Count를 시작값으로 기록하고, 공격이 진행되면서 조기 징후가 실제 데이터 접근으로
> 발전하는 과정과 그에 따른 자동 대응을 확인하겠습니다.
