...
>
> 서로 다른 AWS 로그를 한 화면에서 비교할 수 있어 공격 전후 상황을 조사하기 쉬워졌습니다.
...
>
> GitHub Actions는 정상 서비스를 유지하기 위해 Pod 수를 늘리고, 격리 기능을 켠 뒤 격리할 Pod의 이름과 고유 ID, 사건 해시값을 `values.yaml`에 기록합니다., Argo CD가 이 변경을 EKS에 반영합니다.
>
...
> 이 화면은 두 Rule이 어떤 필드로 연결되는지 확인하기 위해 만든 자료입니다.
> 	
...
> 관제 환경을 구성하면서 가장 먼저 발견한 문제는 Polling 대기 시간이었습니다.
>
> **(5개 Source → Wazuh 10분 Poll)** 프로젝트 초기에는 다섯 종류의 로그를 Wazuh가 10분마다 확인했습니다. 
> **(유지할 가치)** 여러 로그를 넓게 수집하고 원본을 보관하는 데는 좋았지만,
> **(발견한 한계)** 사고가 확인 직후에 발생하면 다음 수집까지 최대 10분을 기다려야 했습니다.
>
> **(실제 개선)** 그래서 기존의 정기 수집 경로는 유지하면서, DVWA의 중요한 사건만 빠르게 보내는 별도 경로를 추가했습니다. 현재 다이어그램의 정기 경로는 5분 Polling으로 줄였고, DVWA 저지연 경로에서는 Poll 대기 자체를 제거했습니다.
>
> 무해한 검증 Event를 세 차례 측정한 결과, 사건 발생부터 Wazuh 경보까지 약 3초에서 6초가 걸렸고 누락은 없었습니다. 이 수치는 공격 전체나 Shuffle 대응 시간까지 측정한 값이 아니라, DVWA Event가 Wazuh 경보로 만들어질 때까지의 지연입니다.
>여기서 기존 경로를 없애지 않은 이유는 원본 증거와 넓은 조사 범위가 계속 필요했기 때문입니다.
> 
> **(평가 기준)** 멘토님과 상담을 통해 1. 속도, 2. 탐지 정확도와 3. 자동 대응 범위 이 셋을 모두 함께 봐야 한다는 것을 알았습니다.
> 탐지 정확도와 자동 대응 범위는 개선하는데 난이도보다는 시간이 문제였습니다. 하지만 속도는 아니였습니다.
> **(맨 밑)** 처음엔 DVWA 저지연 경로에서 시간 단축에 크게 기여한 원리를 **(왼쪽 위)** 나머지 4개 로그 발생지에도 적용하려 하였으나, 몇몇 발생지는 같은 원리를 적용하여도 저희가 원하는 수준 까지 지연을 단축 할 수 없었습니다.
