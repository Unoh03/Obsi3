...
## 2026-06-21 10:29 KST R1 실제 WEB VM 실행 결과
...
`validate-only` 실행에서는 R1 대상 03, 04, 05, 15, 16, 17, 19, 21이 모두 `ready`였고, 나머지 항목은 현재 mode상 `skipped_by_mode`였다. 이는 설정과 mode 경계가 정상이라는 뜻이며 안전 판정은 아니다.

### 실제 실행 판정

| 번호 | 항목 | 결과 | 해석 |
|---:|---|---|---|
| 03 | 디렉터리 인덱싱 | `not_vulnerable` | 후보 디렉터리에서 directory listing 노출 패턴을 찾지 못함 |
| 04 | 에러 페이지 | `not_vulnerable` | 오류 응답에서 설정된 상세 오류 노출 패턴을 찾지 못함 |
| 05 | 정보 노출 | `inconclusive` | 후보 중 200 응답은 있었지만 민감정보 노출 패턴도 403/404 차단 근거도 충분히 확인되지 않음 |
| 15 | 파일 다운로드 | `vulnerable` | profile의 known download candidate가 200으로 직접 내려받아짐 |
| 16 | 불충분한 세션 관리 | `inconclusive` | R1의 비로그인 요청 증거만으로는 인식 가능한 session cookie와 flag를 충분히 확인하지 못함. 로그인 후 세션 변화는 R4 대상 |
| 17 | 데이터 평문 전송 | `vulnerable` | 현재 profile `base_url`이 HTTP이므로 평문 전송 노출을 확인함 |
| 19 | 관리자 페이지 노출 | `not_vulnerable` | profile의 관리자 후보 경로가 직접 노출되지 않음 |
| 21 | 불필요한 Method 악용 | `not_vulnerable` | 위험 HTTP method가 차단 상태로 확인됨 |

### 판정과 다음 단계

- R1은 실제 WEB VM에서 evidence와 report를 생성하는 데 성공했다.
- `inconclusive`인 05와 16은 checker 오류가 아니라, 현재 요청 증거만으로 최종 판정할 근거가 부족하다는 뜻이다.
...
## 2026-06-22 R3 source-assisted fallback 적용 설계

### 목적

R3 대상 07, 09, 11, 12, 13을 현재 `source_assisted_fallback`의 실제 동작과 CARE 소스 구조에 맞춰 분류했다. 이 작업은 HTTP 요청, DB, 로그인, 상태 변경 없이 source evidence의 한계와 다음 구현 순서를 정하는 설계 pass다.

### 확인한 기준 사례와 engine 제약

- 10번은 `member/modify.php`, `member/modifyModel.php`에서 current password 재인증 코드를 읽는 기존 기준 사례다.
- 현재 engine은 지정한 여러 source file을 하나의 문자열로 합쳐 regex를 찾는다.
- `vulnerable_patterns`는 하나라도 일치하면 `vulnerable`을 우선한다.
- 따라서 pattern 부재를 취약 근거로 쓰거나, 취약/조치 branch가 함께 있는 코드를 단일 status로 판정하면 false positive 또는 과장된 안전 판정이 된다.

### 항목별 결정

| 번호 | 결정 | 근거 파일 | source-assisted 결과 한계 |
|---:|---|---|---|
| 07 CSRF | evidence-only, 현재는 `manual_required` | `member/modify.php`, `member/modifyModel.php` | CSRF token hidden field와 서버 token 검증 pattern이 없다. 부재만으로 취약 확정하지 않으며 실제 cross-site 변경은 R4 |
| 09 약한 비밀번호 정책 | evidence-only, 현재는 `manual_required` | `member/registerModel.php`, `vuln/password-recovery/reset.php` | 가입은 필수값/일치만 확인하고, reset safe branch에는 6자 검사가 있다. 모든 비밀번호 변경 경로의 정책을 source만으로 하나의 status로 묶지 않음 |
| 11 불충분한 권한 검증 | R3 첫 구현 대상 | `member/modifyModel.php`, `member/deleteModel.php` | 두 handler가 `$id = $_SESSION['id']`를 쓰고 query도 그 id로 제한한다. 회원 수정·탈퇴의 client-supplied id tampering 범위만 source 근거로 판정 가능 |
| 12 취약한 비밀번호 복구 절차 | 자동 status 보류 | `vuln/password-recovery/request.php`, `verify.php`, `reset.php` | fixed code/화면 노출 취약 branch와 random code/hash/expiry 조치 branch가 공존함 |
| 13 프로세스 검증 누락 | 자동 status 보류 | `vuln/password-recovery/verify.php`, `reset.php` | safe branch의 verified/user/expiry 검증과 vuln branch의 direct reset 허용이 공존함 |

### 공통 rule 결정

source-assisted `not_vulnerable`은 다음을 모두 만족할 때만 허용한다.

1. endpoint와 보안 속성이 하나로 좁혀진다.
2. server-side defense pattern이 긍정적으로 확인된다.
3. DB 상태, 사용자 조합, 취약/조치 mode에 따라 의미가 바뀌지 않는다.
4. `conditions: [source_assisted_evidence_only]`와 좁은 `scope`를 함께 기록한다.

`files`, `not_vulnerable_patterns`, `not_vulnerable_match: all`, `no_match_status: manual_required`, `missing_source_status: manual_required`는 현재 engine으로 사용할 수 있다.

현재 profile에는 WEB VM의 배포 코드 경로만 필요하다.

```yaml
source_root: "/var/www/html/care"
```

07·09·11은 이 값으로 설계 가능하다. 12·13의 `vuln`/`safe` branch 선택값은 engine이 branch를 해석할 수 있을 때만 `source_variant` 같은 profile 값으로 추가한다.

12·13까지 자동화하려면 future engine에 `vulnerable_match: all`, `per_file_patterns`, `source_variant` 또는 branch selector 중 하나 이상이 필요하다. 이번 설계 pass에서는 code/YAML을 수정하지 않는다.

### 다음 구현 순서

1. 11번 회원 수정 endpoint의 session subject binding source check를 먼저 추가한다.
...
- `checker.py`에 generic `workflow_probe` action을 추가했다. engine은 template rendering, 동일 session request sequence, assertion, response extraction, evidence redaction, `fixture_ledger.json`, cleanup failure 처리를 담당한다.
- CARE 전용 URL, form field, fixture 값, 성공 문구는 넣지 않았다. mock profile/check/target으로 분리했다.
- local in-memory mock runtime에서 세 경우를 검증했다.

| mock mode | 기대 결과 | 확인 결과 |
|---|---|---|
| `vulnerable` | controlled probe가 `vulnerable`, cleanup 성공 | 통과 |
| `safe` | controlled probe가 `not_vulnerable`, cleanup 성공 | 통과 |
| `cleanup-fail` | cleanup이 실패하면 전체 `error`, ledger에 실패 기록 | 통과 |

- request/response/ledger에 fixture secret 및 extracted token이 `<REDACTED>`로 남는 것을 확인했다.
- 첫 mock 실행에서 assertion pattern의 `{{fixture_id}}`가 render되지 않는 결함을 발견했고, expect/outcome rule도 실행 시점에 template rendering하도록 수정한 뒤 재검증했다.
- local Python이 `requests` 없이 stdlib fallback으로 실행된 경우, 의도된 cleanup 500 응답에서 `ResourceWarning`이 한 번 남는다. status/ledger assertion은 모두 통과했으며, `HTTPError` handle 정리는 다음 code cleanup으로 남긴다.
- 실제 CARE, WEB VM, DB 대상 HTTP 요청 및 state-changing 실행은 하지 않았다.

### 다음 R4 순서

1. 09, 10, 12, 13의 disposable account workflow YAML을 별도 pass로 설계한다.
2. 실행 전에 fixture 생성·정리 대상과 DB rollback을 사용자와 확인한다.
3. 승인된 `state-changing` run에서만 CARE runtime evidence를 만든다.

## 2026-06-22 R4 Pass 1 보강 및 09번 runtime scaffold

- `workflow_probe`에 step별 `required_mode`를 추가했다. 기존 09~13 source/manual scaffold는 `safe-active`에서 유지하고, runtime workflow만 `state-changing`에서 실행할 수 있다.
- `when`, `set_context`, outcome별 `set_context`를 추가했다. 예를 들어 09번에서 약한 비밀번호 가입이 방어되어 계정이 생성되지 않으면 로그인·탈퇴 cleanup을 기록상 `skipped`로 남기고 요청하지 않는다.
- runtime workflow가 실제 `vulnerable` 또는 `not_vulnerable` verdict를 만들면 같은 check의 `manual_required` scaffold보다 우선한다. runtime `inconclusive`는 기존 보조 evidence를 유지한다.
- stdlib HTTP fallback에도 cookie jar를 추가해 `requests`가 없어도 login session을 다음 workflow step에 전달하도록 했다.
- local-only mock regression은 5개를 통과했다. 취약/방어/cleanup 실패/step mode gate/stdlib cookie session을 확인했다.
- 09번 check에 weak-password registration runtime workflow를 추가했다. run별 계정으로 `1234`를 등록 시도하고, 성공했을 때만 login 후 deleteModel.php로 해당 계정을 제거한다.
- `python -B checker.py --profile profiles/care.yml --checks checks --mode state-changing --validate-only --check-id 09`는 `ready`를 확인했다.
- 실제 CARE HTTP 요청, DB 계정 생성·삭제, WEB VM 실행은 하지 않았다. 09번 runtime run은 별도 승인과 rollback 확인 후에만 수행한다.

## 2026-06-22 09·10·12·13 runtime workflow scaffold 준비

- `profiles/care.yml`에 run fixture 전용 register/login/modify/delete/recovery route를 추가했다. 기존 static route의 고정 예시 data를 runtime workflow가 재사용하지 않도록 분리했다.
- 09: `1234` 약한 비밀번호 가입을 probe로 보며, 가입 성공 시에만 login 후 deleteModel.php cleanup을 실행한다. 향후 정책이 가입을 거부하면 cleanup도 `skipped`로 남긴다.
- 10: run fixture account를 로그인한 뒤 `currentPw` 없이 modifyModel.php를 호출한다. 차단되면 `not_vulnerable`, 수정되면 `vulnerable`이며, 수정된 경우 cleanup에 쓸 비밀번호도 outcome context로 바꾼다.
- 12: vulnerable recovery request가 `Issued Code`와 웹 노출 문구를 반환하는지 확인한다. safe branch의 shared `/tmp` mailbox append는 자동 cleanup이 불가능하므로 현재 runtime 범위에서 제외했다.
- 13: vulnerable reset branch에 request/verify 없이 직접 POST해 비밀번호 변경 문구가 나오는지 확인한다. 성공하면 changed password로 login 후 fixture 계정을 삭제한다.
- 네 check 모두 `state-changing` step으로만 추가했으며, `--validate-only --check-id 09,10,12,13`에서 `ready`를 확인했다.
- local workflow regression은 5개를 통과했다. no `ResourceWarning`; stdlib cookie jar session 유지도 검증했다.
- 실제 CARE HTTP 요청, DB account 생성/변경/삭제, cleanup은 아직 실행하지 않았다.

## 2026-06-22 06·07·11·14 source 재점검

- 06 Stored XSS: `center/writeModel.php`가 게시글을 INSERT하고 `view.php`가 `<?=$content?>`으로 escape 없이 출력한다. 그러나 `center/`에는 게시글 단건 delete endpoint가 없다. 자동 fixture post를 만들면 rollback을 보장할 수 없어 runtime workflow를 보류한다.
- 14: `writeModel.php`는 `multipart/form-data`, DB row, `/data/` file을 함께 만든다. 방어 코드가 위쪽에 있어도 하단의 `$upfile = $_FILES['upfile']['name']`와 `move_uploaded_file()`가 다시 실행되어 현재 취약 흐름을 덮어쓴다. multipart engine과 게시글/파일 cleanup 계약 없이는 자동 upload를 실행하지 않는다.
- 07: 현재 `modifyModel.php`에 CSRF token 검증은 없지만 `currentPw` 재인증은 활성화되어 있다. token 없는 변경이 현재 비밀번호 요구로 실패해도, 그것을 CSRF 정석 방어 완료라고 판정하면 안 된다.
- 11: 회원 수정의 session subject binding은 source로 확인되지만, board/download에는 private ownership model과 A/B session 검증 대상이 없다. 현재 전체 권한 검증 runtime은 만들지 않는다.
- 다음 구현은 06/14 cleanup path와 11 multi-session/private-object 요구를 별도 설계한 뒤 진행한다. 실제 CARE/DB 변경은 여전히 미실행이다.
