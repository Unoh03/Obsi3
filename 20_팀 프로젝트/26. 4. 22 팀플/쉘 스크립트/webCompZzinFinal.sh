...
# - Tomcat 설치, GitHub repo pull/build, ROOT.war 배포, DB/Gmail/Redis 설정 분리, NFS upload mount, 기본 검증을 한 번에 수행한다.
#
# 실행 위치:
...
# - DB/Gmail secret 분리
# - Redis 세션 설정 및 redis-cli 연결 확인
# - NFS VIP upload mount
#
# 실행 예시:
#   sudo bash 'web-comp_final.sh'
#   sudo PROJECT_DIR=/home/t_web/zzaphub RUN_PROMTAIL=0 bash 'web-comp_final.sh'
#   sudo PROMTAIL_MODE=prompt bash 'web-comp_final.sh'
#
# 자동화/비대화형 실행 예시:
...
#        REDIS_HOST='192.168.2.7' REDIS_PORT='6379' \
#        bash 'web-comp_final.sh'
#
...
# Gmail SMTP 인증 메일은 외부 인터넷 경로가 필요하다.
# 내부망 default gateway가 잡혀 있어도, WEB 서버가 직접 생성하는 tcp/587 트래픽만 ens33으로 우회시킨다.
RUN_SMTP587_ROUTE="${RUN_SMTP587_ROUTE:-1}"
SMTP587_REQUIRED="${SMTP587_REQUIRED:-1}"
SMTP587_PERSIST="${SMTP587_PERSIST:-1}"
SMTP587_DEV="${SMTP587_DEV:-ens33}"
SMTP587_GW="${SMTP587_GW:-192.168.10.2}"
SMTP587_TABLE_ID="${SMTP587_TABLE_ID:-587}"
SMTP587_MARK_ID="${SMTP587_MARK_ID:-587}"
SMTP587_PRIORITY="${SMTP587_PRIORITY:-100}"
SMTP587_HOST="${SMTP587_HOST:-smtp.gmail.com}"
SMTP587_SERVICE_NAME="${SMTP587_SERVICE_NAME:-zzaphub-smtp587-policy-route.service}"
...
print_missing_secret_help() {
    ...
    warn "방법 1: 터미널에서 sudo bash 'web-comp_final.sh' 로 실행해 질문에 답하세요."
...
verify_secure_patch() {
    ...
    if [ "${failed}" -ne 0 ]; then
        die "DB/Gmail/Redis 설정 내장 통합 검증 실패"
    fi

    log "DB/Gmail secret 분리 및 Redis 세션 설정 검증 완료"
}

install_packages() {
    log "필수 패키지를 설치합니다."
    apt update
    apt install -y openjdk-17-jdk curl wget tar redis-tools iptables
}
...
run_secure_patch() {
    if [ "${RUN_SECURE}" != "1" ]; then
        warn "RUN_SECURE=0 이므로 DB/Gmail secret 분리와 Redis 세션 설정 보정을 건너뜁니다."
        return 0
    fi
...
smtp587_mark_hex() {
    printf '0x%x' "${SMTP587_MARK_ID}"
}

apply_smtp587_policy_route() {
    local mark_hex

    if [ "${RUN_SMTP587_ROUTE}" != "1" ]; then
        warn "RUN_SMTP587_ROUTE=0 이므로 SMTP 587 정책 라우팅을 건너뜁니다."
        return 0
    fi

    mark_hex="$(smtp587_mark_hex)"

    log "SMTP 587 정책 라우팅 적용: tcp/587 -> fwmark ${SMTP587_MARK_ID} -> table ${SMTP587_TABLE_ID} -> ${SMTP587_GW} dev ${SMTP587_DEV}"

    if ! ip link show "${SMTP587_DEV}" >/dev/null 2>&1; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 실패: ${SMTP587_DEV} 인터페이스가 없습니다."
        fi
        warn "SMTP 587 우회 건너뜀: ${SMTP587_DEV} 인터페이스가 없습니다."
        return 0
    fi

    if ! command -v iptables >/dev/null 2>&1; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 실패: iptables 명령을 찾지 못했습니다."
        fi
        warn "SMTP 587 우회 건너뜀: iptables 명령을 찾지 못했습니다."
        return 0
    fi

    ip route replace default via "${SMTP587_GW}" dev "${SMTP587_DEV}" table "${SMTP587_TABLE_ID}"

    if ! ip rule list | grep -Eq "fwmark (${SMTP587_MARK_ID}|${mark_hex}).*lookup ${SMTP587_TABLE_ID}"; then
        ip rule add fwmark "${SMTP587_MARK_ID}" table "${SMTP587_TABLE_ID}" priority "${SMTP587_PRIORITY}"
    fi

    iptables -t mangle -C OUTPUT -p tcp --dport 587 -j MARK --set-mark "${SMTP587_MARK_ID}" 2>/dev/null \
        || iptables -t mangle -A OUTPUT -p tcp --dport 587 -j MARK --set-mark "${SMTP587_MARK_ID}"

    log "SMTP 587 정책 라우팅 런타임 적용 완료"
}

email_verify_enable() {
    local service_file="/etc/systemd/system/${SMTP587_SERVICE_NAME}"
    local mark_hex
    ...
    if [ "${SMTP587_PERSIST}" != "1" ]; then
        warn "SMTP587_PERSIST=0 이므로 재부팅 후 자동 복원 service 생성을 건너뜁니다."
        return 0
    fi
    ...
    if [ -f "${service_file}" ]; then
        log "기존 SMTP 587 systemd service 백업: $(backup_file_or_dir "${service_file}")"
    fi

    log "SMTP 587 정책 라우팅 systemd service 작성: ${service_file}"

    cat > "${service_file}" <<EOF
[Unit]
Description=Zzaphub SMTP 587 policy route via ${SMTP587_DEV}
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/bash -lc 'ip link show "${SMTP587_DEV}" >/dev/null 2>&1'
ExecStart=/bin/bash -lc 'ip route replace default via "${SMTP587_GW}" dev "${SMTP587_DEV}" table "${SMTP587_TABLE_ID}"'
ExecStart=/bin/bash -lc 'ip rule list | grep -Eq "fwmark (${SMTP587_MARK_ID}|${mark_hex}).*lookup ${SMTP587_TABLE_ID}" || ip rule add fwmark "${SMTP587_MARK_ID}" table "${SMTP587_TABLE_ID}" priority "${SMTP587_PRIORITY}"'
ExecStart=/bin/bash -lc 'iptables -t mangle -C OUTPUT -p tcp --dport 587 -j MARK --set-mark "${SMTP587_MARK_ID}" 2>/dev/null || iptables -t mangle -A OUTPUT -p tcp --dport 587 -j MARK --set-mark "${SMTP587_MARK_ID}"'
RemainAfterExit=yes
...
EOF
    ...
    chmod 644 "${service_file}"
    systemctl daemon-reload
    systemctl enable --now "${SMTP587_SERVICE_NAME}"

    log "SMTP 587 정책 라우팅 systemd service 활성화 완료"
}

verify_smtp587_policy_route() {
    local smtp_ip
    local mark_hex
    ...
    log "SMTP 587 정책 라우팅 검증"

    if ! ip rule list | grep -Eq "fwmark (${SMTP587_MARK_ID}|${mark_hex}).*lookup ${SMTP587_TABLE_ID}"; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 검증 실패: ip rule에서 fwmark ${SMTP587_MARK_ID} -> table ${SMTP587_TABLE_ID} 규칙을 찾지 못했습니다."
        fi
        warn "SMTP 587 우회 검증 실패: ip rule 규칙을 찾지 못했습니다."
        return 0
    fi

    if ! ip route show table "${SMTP587_TABLE_ID}" | grep -Eq "default via ${SMTP587_GW} dev ${SMTP587_DEV}"; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 검증 실패: routing table ${SMTP587_TABLE_ID}의 default route가 기대값과 다릅니다."
        fi
        warn "SMTP 587 우회 검증 실패: routing table ${SMTP587_TABLE_ID} default route 확인 필요"
        return 0
    fi

    if ! iptables -t mangle -C OUTPUT -p tcp --dport 587 -j MARK --set-mark "${SMTP587_MARK_ID}" 2>/dev/null; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 검증 실패: iptables mangle OUTPUT mark rule을 찾지 못했습니다."
        fi
        warn "SMTP 587 우회 검증 실패: iptables mark rule 확인 필요"
        return 0
    fi

    smtp_ip="$(getent ahostsv4 "${SMTP587_HOST}" | awk 'NR==1{print $1}' || true)"
    if [ -z "${smtp_ip}" ]; then
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 우회 검증 실패: ${SMTP587_HOST} IPv4 DNS 조회 실패"
        fi
        warn "SMTP 587 우회 검증 건너뜀: ${SMTP587_HOST} IPv4 DNS 조회 실패"
        return 0
    fi

    log "SMTP route check: ${SMTP587_HOST}=${smtp_ip}"
    ip -4 route get "${smtp_ip}" mark "${SMTP587_MARK_ID}" || true

    if timeout 5 bash -c "</dev/tcp/${smtp_ip}/587"; then
        log "SMTP 587 TCP 연결 확인 완료: ${smtp_ip}:587"
    else
        if [ "${SMTP587_REQUIRED}" = "1" ]; then
            die "SMTP 587 TCP 연결 실패: ${smtp_ip}:587. ens33 NAT, DNS, 외부망 연결을 확인하세요."
        fi
        warn "SMTP 587 TCP 연결 실패: ${smtp_ip}:587"
    fi
}

setup_smtp587_policy_route() {
    if [ "${RUN_SMTP587_ROUTE}" != "1" ]; then
        warn "RUN_SMTP587_ROUTE=0 이므로 SMTP 587 정책 라우팅을 건너뜁니다."
        return 0
    fi
...
prompt_switch_to_internal_network() {
    ...
    echo "[INFO] ens33 인터넷 어댑터는 유지하고, 내부망 어댑터를 다시 연결하세요."
    echo "[INFO] 이후 SMTP 587 트래픽만 ens33으로 우회 라우팅합니다."
    echo "================================================================"
    read -r -p "준비되면 엔터: "
}
...
main() {
    ...
    log "RUN_SMTP587_ROUTE=${RUN_SMTP587_ROUTE}, SMTP587_DEV=${SMTP587_DEV}, SMTP587_GW=${SMTP587_GW}, SMTP587_PERSIST=${SMTP587_PERSIST}"
    ...
    verify_redis_connection       # Redis 세션 저장소 연결 확인
