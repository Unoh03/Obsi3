...
# - Tomcat 설치, GitHub repo pull/build, ROOT.war 배포, DB secret 분리, NFS upload mount, 기본 검증을 한 번에 수행한다.
#
# 실행 위치:
...
#
# 실행 예시:
#   sudo PROJECT_DIR=/home/t_web/zzaphub RUN_PROMTAIL=0 bash 'web(5.6).sh'
...
is_placeholder() {
    case "$1" in
        '${MASTER_DB_URL}'|'${MASTER_DB_USER}'|'${MASTER_DB_PASSWORD}'|'${SLAVE_DB_URL}'|'${SLAVE_DB_USER}'|'${SLAVE_DB_PASSWORD}')
            return 0
            ;;
        *)
            return 1
...
EOF
    ...
    if [ "${missing}" -ne 0 ]; then
        die "${ENV_FILE} 에 MASTER_DB_*/SLAVE_DB_* 6개 값을 채울 수 없습니다. 환경변수로 넘기거나 파일을 직접 작성하세요."
    fi
...
secure_application_properties() {
    ...
    log "application.properties 의 master/slave DB 접속정보를 환경변수 placeholder 로 변경했습니다."
}
