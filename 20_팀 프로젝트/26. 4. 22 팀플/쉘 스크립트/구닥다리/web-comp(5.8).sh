...
#
# 실행 예시:
#   sudo bash 'web-comp(5.8).sh'
#   sudo PROJECT_DIR=/home/t_web/zzaphub RUN_PROMTAIL=0 bash 'web-comp(5.8).sh'
#   sudo PROMTAIL_MODE=prompt bash 'web-comp(5.8).sh'
#
# 자동화/비대화형 실행 예시:
...
print_missing_secret_help() {
    ...
    warn "방법 1: 터미널에서 sudo bash 'web-comp(5.8).sh' 로 실행해 질문에 답하세요."
...
run_promtail_client() {
    if [ "${RUN_PROMTAIL}" != "1" ] || [ "${PROMTAIL_MODE}" = "skip" ]; then
        warn "Promtail 로그 수집 설정을 건너뜁니다."
        return 0
    fi

    if ( run_promtail_client_impl ); then
    ...
    warn "Promtail 설정 실패. WEB 서비스 자체는 계속 검증합니다."
}
