...
`AWS기초.pdf` 기반 학원 AWS 수업, 공식 Arc 과정, IaC / Terraform을 서로 구분해 연결하는 AWS 영역의 진입점이다.

공식 Arc 과정은 현재 RAW와 원본 PDF까지만 라우팅한다. 승인된 안정 학습 노트가 생기기 전에는 Source Digest를 현재 지식으로 사용하지 않는다.
...
- 현재 수업 필기: [[10_학습 노트/클라우드/공식 Arc 과정/raw 메모|공식 Arc 과정 RAW 메모]] - 현재진행 원문 기록이며 완료 정리본으로 단정하지 않는다.
- 공식 원본: [[40_자료/강의 자료/AWS Arc/README|Architecting on AWS 원본 PDF 목차]] - 개요, 모듈 1~13, 교육 과정 요약 PDF로 바로 이동한다.

> [!warning] 승인되지 않은 파생물
> `40_자료/강의 자료/AWS Arc/`의 Source Digest는 OCR·AI 변환 초안이다. 삭제하지 않고 보존하지만, 사용자가 승인하기 전에는 공식 원본이나 안정 학습 노트로 취급하지 않는다.
...
- [ ] p.123-126 확장 주제는 학원 AWS기초 흐름의 후속 후보로 남긴다.
...
- 공식 Arc 과정의 현재진행 RAW·원본 PDF와 학원 AWS기초 흐름을 섞지 않는다.
