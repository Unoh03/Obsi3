...
따라서 `network.tf`, `database.tf`, `autoscaling.tf`는 서로 다른 State가 아니다. 파일명은 사람이 읽기 위한 구분일 뿐이다.
...
두 파일 모두 문법 검사를 통과했다. 실제 EC2에서 실행한 결과는 아직 아니다.
...
`terraform plan`은 성공했지만 다음은 아직 검증하지 않았다.
...
특히 RDS Multi-AZ와 ALB는 짧은 실습이라도 별도 비용이 발생한다. 따라서 현재 Goal에서는 다음 경계를 유지한다.
...
Apply 후에는 같은 날 기능 검증과 `terraform destroy`까지 수행하는 것이 비용상 안전하다.

## 39. 사람이 검토할 권장 순서

파일명 순서대로 읽기보다 Architecture의 계약 순서로 읽는 편이 이해하기 쉽다.
...
2. locals.tf · data.tf
   → 계산값, 현재 Public IP, AMI, 기존 Key Pair
...
핵심은 `autoscaling.tf`를 먼저 읽지 않는 것이다. ASG는 Network·SG·S3·IAM·RDS·ALB Output을 한꺼번에 소비하므로 앞의 계약을 먼저 알아야 읽기 쉽다.
...
| Apply | 미수행 | 비용 승인 전 |
| Runtime 검증 | 미수행 | AWS Resource 미생성 |
| Destroy | 해당 없음 | 아직 생성하지 않음 |

현재 v17은 **실행 가능한 Plan을 만든 상태**다. 실제 Architecture가 정상 동작한다고 완료 판정할 단계는 아니다.

### 다음 재시작 지점

```powershell
cd D:\terraform\workspace\17_external_module
Copy-Item terraform.tfvars.example terraform.tfvars
```

실제 Lab Password를 검토한 뒤:
...
Apply 직전에는 예상 비용과 `65 to add` 범위를 다시 확인한다.

## 41. v17 한 문단 요약

v17에서는 `terraform-aws-modules`의 VPC·EC2·Security Group·ALB·Auto Scaling·RDS·S3·IAM Module을 한 Root Module에서 조립하여 2-AZ Public ALB, Private ASG Application, 격리된 Multi-AZ MariaDB Architecture를 설계했다. NAT Gateway는 비용 때문에 비활성화하고, v16에서 검증한 Ubuntu NAT Instance 설정과 직접 `aws_route`를 결합했다. ALB는 Target Group을 만들되 고정 Instance를 등록하지 않고, ASG가 Launch Template과 Target 등록 및 App Instance 수명주기를 소유한다. RDS Endpoint, S3 Bucket, IAM Instance Profile, SG ID, Subnet ID는 Module Output에서 ASG Input으로 연결된다. 최신 RDS Module의 `password_wo` 변경과 Plan 시점에 알 수 없는 ID를 `for_each` Key로 사용한 문제를 실제 Init·Plan에서 발견해 수정했으며, 최종적으로 `validate`와 `Plan: 65 to add, 0 to change, 0 to destroy`까지 통과했다. Apply와 Runtime 검증은 비용 승인 전이라 아직 수행하지 않았다.
