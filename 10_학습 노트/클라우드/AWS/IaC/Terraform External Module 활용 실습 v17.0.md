...
| 검증 항목 | 결과 | 관찰 근거 |
|---|---|---|
| NAT Instance | 완료 | `ap-northeast-2a`, `running`, Public IP 발급 |
...
Private App은 NAT Route를 통해 Package·AWS CLI를 설치하고 S3에서 `boot.war`를 받은 뒤, RDS 연결과 Schema 초기화를 통과해야 Tomcat Application을 배포한다. 두 Target의 `healthy`와 ALB HTTP 200은 이 전체 경로가 동작했음을 강하게 뒷받침한다. 다만 NAT 내부의 `net.ipv4.ip_forward`와 `iptables MASQUERADE` 값 자체는 아직 SSH로 직접 조회하지 않았다.
...
Apply와 기능 검증은 완료했다. 현재 Resource는 실행 중이므로 같은 날 `terraform destroy`하는 것이 비용상 안전하다. Destroy는 별도의 파괴적 작업이므로 사용자 명시 승인 전에는 실행하지 않는다.
...
| Destroy | 미수행 | Resource 실행 중, 별도 승인 필요 |

현재 v17은 **계획한 Architecture를 실제 생성하고 핵심 Service 경로까지 검증한 상태**다. 남은 작업은 선택적인 NAT 내부 직접 확인과 비용 방지를 위한 Destroy다.
...
실습 종료 후에는 사용자 명시 승인 뒤 다음을 실행한다.
