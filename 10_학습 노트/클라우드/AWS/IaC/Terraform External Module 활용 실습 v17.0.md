...
현재 v17은 **계획한 Architecture를 실제 생성하고 핵심 Service 경로 및 NAT 내부 설정까지 검증한 상태**다. 남은 작업은 비용 방지를 위한 Destroy다.
