...
source:
  ...
  - 17_external_module(답지)/main.tf
  - 17_external_module(답지)/variables.tf
  - 17_external_module(답지)/nat_install.tpl
  - 17_external_module(답지)/web_install.tpl
  - 17_external_module(답지)/local_module/ami/*.tf
  - 17_external_module(답지)/local_module/s3/*.tf
  - 17_external_module(답지v2)/main.tf
  - 17_external_module(답지v2)/local_module/ami/*.tf
  - 17_external_module(답지v2)/local_module/s3/*.tf
  - HashiCorp Terraform Provisioner·timestamp·pathexpand·Module Version 공식 문서
  - AWS Auto Scaling Target Tracking·ALB Access Log·RDS Multi-AZ 공식 문서
...
evidence:
  ...
  - v17 Apply 후 ASG 2대·ALB Target 2대 Healthy 및 HTTP 200
  - v17 RDS Multi-AZ·비공개·암호화와 Route 경계 확인
  - NAT IP Forwarding·MASQUERADE·영속화 직접 확인
  - "Destroy complete! Resources: 65 destroyed."
  - Terraform State EMPTY 및 주요 AWS 잔존 Resource 없음
  - 강사 답지 v1→v2 Source Diff와 우리 구현의 HCL Block·흐름·Artifact Hash 정적 대조
  - 강사 답지 v2 ZIP과 압축 해제 폴더의 파일별 SHA-256 일치
  - 강사 답지 v1 terraform fmt -check -recursive 통과
  - 강사 답지 v2 terraform fmt -check -recursive 미통과(main.tf Format 차이)
...
# Terraform External Module 활용 실습 v17.1

> [!summary]
> Part 1~5는 v16에서 Registry VPC Module과 Local NAT·Servers Module을 조립하고 실제 SSH·NAT 통신까지 검증한 기록이다. Part 6은 v17에서 VPC·EC2·Security Group·ALB·ASG·RDS·S3·IAM을 Registry Module 중심으로 조립하고, 65개 Resource의 Apply·Runtime·Destroy까지 검증한 기록이다. Part 7은 최신 강사 답지 v2와 우리 구현을 정적 대조하고, v1에서 v2로 수정된 지점까지 추적하여 Custom AMI 방식과 Boot-time `user_data` 방식의 흐름, 보안·운영·재현성 차이, 채택할 학습점을 정리한다.
...
- [[#42. v17.1 비교 목적과 범위|v17.1 비교 범위]]
- [[#46. 두 구현의 전체 작동 흐름|두 구현 흐름]]
- [[#55. 답지 코드의 확인 필요 지점|답지 확인 지점]]
- [[#57. 무엇을 배우고 무엇을 채택할 것인가|채택 판단]]
- [[#59. v17.1 완료 판정|v17.1 완료 판정]]
...
아래 목록은 Part 6의 Plan 작성 시점에 남아 있던 항목이다. 이후 같은 Part의 Runtime·Destroy 절에서 실제 검증을 완료했다.
...
# Part 7. v17.1 강사 답지 대조와 흐름 분석

## 42. v17.1 비교 목적과 범위

v17.0에서는 강사의 설계 화면과 사용자 결정을 바탕으로 External Registry Module 중심의 Architecture를 독립적으로 구현했다. 이후 강사 답지가 두 차례 공개됐으며, v17.1의 최종 판정은 최신 `17_external_module(답지v2)`를 기준으로 한다. 최초 답지는 v1→v2 수정 흐름을 확인하는 비교 자료로만 남긴다.

```text
비교 대상 A
D:\terraform\workspace\17_external_module
→ 우리 구현
→ 실제 Init·Validate·Plan·Apply·Runtime·Destroy 완료

비교 대상 B
D:\terraform\workspace\17_external_module(답지v2)
→ 강사 최신 답지 v2
→ 소스 정적 검토 수행
→ fmt -check는 main.tf Format 차이로 미통과

변경 이력 비교
D:\terraform\workspace\17_external_module(답지)
→ 최초 답지 v1
→ v2에서 수정된 항목을 판별하는 기준
```

비교의 목적은 답지와 글자 단위로 같게 만드는 것이 아니다.

```text
1. 공통 Architecture를 확인한다.
2. Resource가 생성되는 전체 흐름을 비교한다.
3. 답지가 추가로 가르치려는 개념을 추출한다.
4. 실행상 전제·오류 가능성·보안 차이를 분리한다.
5. 우리 구현에 채택할 부분과 보류할 부분을 결정한다.
```

답지는 강사의 의도를 보여주는 고신뢰 수업 자료지만, 코드 자체의 실행 성공을 자동으로 보장하는 증거는 아니다. 우리 코드도 답지와 다르다는 이유만으로 틀린 것이 아니며 실제 Runtime 증거를 함께 기준으로 판단한다.

## 43. 정적 Diff 개요

두 폴더는 파일 구성이 처음부터 다르므로 일반적인 Line Diff만으로는 의미를 읽기 어렵다. HCL Block과 Resource 소유권, Output→Input 연결을 기준으로 비교했다.

| 항목 | 우리 구현 | 강사 답지 |
|---|---:|---:|
| `.tf` 파일 | 5개 | 8개 |
| `.tf` 총 행 수 | 615 | 472 |
| `module` Block | 14개 | 13개 |
| Root·Local 직접 `resource` Block | 1개 | 9개 |
| Root·Local `output` Block | 10개 | 5개 |
| Registry Module Version 명시 | 전부 명시 | 미명시 |
| 실제 Apply·Runtime 검증 | 완료 | 미수행 |

> [!note]
> 위 개수는 소스의 최상위 Block을 문자열 기준으로 센 값이다. Registry Module 내부 Resource 개수와 실제 AWS Resource 개수를 뜻하지 않는다.

### 43-1. 답지 v1→v2에서 직접 수정된 항목

| v1의 상태 | v2 변경 | 판정 변화 |
|---|---|---|
| Bastion Instance ID를 SSH Host로 전달 | `module.bastion_instance.public_ip`로 변경 | 연결 오류 후보 해소 |
| S3 Instance Profile Output 미사용 | Seed Web EC2의 `iam_instance_profile`에 연결 | `aws s3 cp` Credential 경로 연결 |
| ALB Target Group에 고정 `target_id` | 해당 줄 제거 | ASG Attachment와의 역할 혼선 해소 |
| ASG Launch Template SG 미명시 | `security_groups`에 Web SG 연결 | ASG Instance의 8080·SSH 규칙 연결 |
| RDS `family = "default.mariadb11.8"` | `family = "mariadb11.8"` | Parameter Group Family 형식 수정 |
| ALB Access Log 외부 Bucket 참조 | `access_logs` 제거 | `my-alb-logs` 선행 조건 제거 |
| 고정 이름 다수 | `${var.env}` Prefix 적용 | 환경 간 이름 구분 개선 |
| 운영 확인용 Root Output 없음 | Web Private IP·Bastion Public IP 공개 | 접속·검증 편의 개선 |

이 변경은 최초 답지에 대한 정적 검토에서 발견했던 연결 문제 중 여러 항목을 강사가 후속 수정했다는 뜻이다. 따라서 아래 비교에서는 **v2에서 고쳐진 항목을 현재 결함으로 취급하지 않는다.**

두 폴더의 `boot.war`는 다음 SHA-256으로 동일했다.

```text
53D09CECEE76279BE6B31B4A4A14FF5C512FC1301B6C5EC492583ABE752510DB
```

따라서 Application Artifact가 달라서 흐름이 갈린 것은 아니다. 차이는 Artifact를 Instance에 넣는 **시점과 Image 공급 방식**이다.

## 44. 공통 Architecture

두 구현은 큰 틀에서 다음 구조를 공유한다.
...
│  └─ ASG Application Instance
└─ Private Database Subnet 2개
   └─ RDS MariaDB
...
- VPC·EC2·RDS·Security Group·ALB·ASG에 Registry Module을 사용한다.
- NAT Gateway 대신 EC2 NAT Instance를 사용한다.
- ALB가 Port 8080의 Application Target Group으로 전달한다.
- ASG가 두 Private Subnet에 Application Instance를 배치한다.
- RDS는 Database Subnet을 사용한다.
- S3에 `boot.war`를 저장한다.
- Target Group의 고정 Attachment를 만들지 않고 ASG가 Target 등록을 소유한다.

## 45. 가장 큰 차이: Application Image를 언제 만드는가

### 우리 구현: Boot-time Provisioning

우리 ASG는 Ubuntu Base AMI로 Instance를 만든 뒤 각 Instance의 `user_data`가 Application을 설치한다.

```text
ASG Scale-out
→ Ubuntu Instance 생성
→ NAT를 통해 Package·AWS CLI 설치
...
- 별도 Seed Instance와 Custom AMI 수명주기가 없다.
- Terraform Graph가 비교적 단순하다.
- Base AMI와 Script를 바꾸면 Launch Template Version으로 반영할 수 있다.
- 우리가 실제 Apply와 재실행 `No changes`까지 검증했다.

단점:

- Scale-out 때마다 Package 설치와 Artifact Download를 반복한다.
- Instance가 `healthy`가 되기까지 시간이 더 걸린다.
- 부팅 시 NAT·Package Repository·S3·RDS가 모두 정상이어야 한다.

### 강사 답지: Seed Instance → Custom AMI

답지는 먼저 Web EC2 한 대를 완성한 뒤 그 Instance에서 AMI를 만든다.

```text
Web Seed Instance 생성
→ web_install.tpl 실행
→ Bastion에서 Private Web HTTP 응답 대기
→ aws_ami_from_instance로 Custom AMI 생성
→ AWS CLI로 Seed Instance 종료
→ ASG Launch Template에 Custom AMI 전달
→ ASG Instance 생성
```

장점:

- ASG Instance는 이미 설치된 Image로 시작하므로 부팅이 빠를 수 있다.
- 같은 Image ID에서 시작하므로 Instance 사이 결과가 더 균일하다.
- Golden Image 또는 Immutable Infrastructure의 기초 개념을 보여준다.

단점:

- Seed Instance·AMI·Provisioner라는 별도 수명주기가 생긴다.
- AMI 갱신·정리·Version 관리가 필요하다.
- 현재 답지의 Provisioner와 `timestamp()` 구현은 반복 Apply의 수렴성을 해칠 수 있다.
...
```mermaid
flowchart TD
    ROOT["Root Module"] --> DATA["Account·Public IP·Ubuntu AMI·Key Pair 조회"]
    DATA --> VPC["Registry VPC Module"]
    VPC --> SG["Registry Security Group Modules"]
    VPC --> NAT["NAT Instance"]
    VPC --> BASTION["Bastion"]
    NAT --> ROUTE["App Private Route → NAT ENI"]

    VPC --> S3["Registry S3 Bucket·Object Modules"]
    S3 --> IAM["Registry IAM Role Module"]
    VPC --> RDS["Registry RDS Module\nMulti-AZ·Private"]
    VPC --> ALB["Registry ALB Module\nPublic 2-AZ"]

    ROUTE --> ASG["Registry ASG Module"]
    SG --> ASG
    IAM --> ASG
    RDS -->|"Endpoint"| ASG
    S3 -->|"Bucket ID"| ASG
    ...
    ASG --> LT["Launch Template\nUbuntu + web_install.sh"]
    LT --> APP1["Private App 2a"]
    LT --> APP2["Private App 2c"]
    APP1 --> TARGET["ALB Target Group"]
    APP2 --> TARGET
```

핵심은 ASG가 Base AMI와 `user_data`를 동시에 소유한다는 점이다. RDS Endpoint·S3 Bucket·IAM Instance Profile·SG·Subnet·Target Group ARN이 모두 ASG Input으로 모인다.

### 46-2. 강사 답지

```mermaid
flowchart TD
    ROOT["Root Module"] --> VPC["Registry VPC Module"]
    VPC --> SG["Registry Security Group Modules"]
    VPC --> NAT["NAT Instance"]
    VPC --> BASTION["Bastion"]
    NAT --> ROUTE["Private Route → NAT ENI"]

    VPC --> RDS["Registry RDS Module"]
    ROOT --> S3LOCAL["Local S3 Module\nBucket·Object·IAM"]
    VPC --> SEED["Seed Web Instance"]
    SG --> SEED
    RDS -->|"Endpoint"| SEED
    S3LOCAL -->|"Bucket ID"| SEED

    BASTION --> WAIT["terraform_data remote-exec\nPrivate Web HTTP 대기"]
    SEED --> WAIT
    WAIT --> AMI["aws_ami_from_instance"]
    AMI --> TERMINATE["local-exec\nSeed EC2 강제 종료"]

    AMI --> ASG["Registry ASG Module\nCustom AMI 사용"]
    VPC --> ASG
    ASG --> TARGET["ALB Target Group"]
    TARGET --> ALB["ALB HTTPS Listener"]
    ALB --> DNS["Route 53 www Record"]
```

답지에서는 `module.ami.web-ami-id`가 ASG의 `image_id`로 들어간다. 따라서 Seed Web Instance가 준비되지 않으면 AMI와 ASG도 진행할 수 없다.

## 47. Resource 소유권 차이

| 책임 | 우리 구현 | 강사 답지 |
|---|---|---|
| VPC·Subnet·Route Table | Registry VPC Module | Registry VPC Module |
| NAT·Bastion | Registry EC2 Module | Registry EC2 Module |
| Seed Web EC2 | 없음 | Registry EC2 Module |
| App Launch Template | Registry ASG Module | Registry ASG Module |
| App 초기화 | ASG `user_data` | Seed Web `user_data` 후 AMI |
| Custom AMI | 없음 | Local AMI Module |
| S3 Bucket·Object | Registry S3 Module | Local S3 Module |
| IAM Role·Profile | Registry IAM Module | Local S3 Module 내부 직접 Resource |
| Security Group | 역할별 Registry SG Module | 역할별 Registry SG Module, ALB SG는 ALB Module 내부 |
| NAT Route | Root `aws_route` | Root `aws_route` |
| RDS | Registry RDS Module | Registry RDS Module |
| ALB·Target Group | Registry ALB Module | Registry ALB Module |
| DNS·Certificate | 제외 | Route 53·ACM Data Source 포함 |

우리 Root Module은 External Module의 Input과 Output을 조립하는 성격이 강하다. 답지는 External Module을 사용하면서도 Image 제작과 S3·IAM을 Local Module로 직접 구현한 **혼합형 구성**이다.
...
| NAT 위치 | Public 2a | Public 2c |
| Bastion 위치 | Public 2c | Public 2a |
| App Route 반복 | 안정된 Key Map `for_each` | Route Table 수 기반 `count` |
| DB Route 격리 | NAT·IGW 기본 경로 생성을 명시적으로 차단 | 별도 기본 Route를 작성하지 않음 |
| NAT Script | IP Forwarding·MASQUERADE·영속화 | IP Forwarding·MASQUERADE만 |
| NAT 직접 검증 | SSH로 완료 | 실행 증적 없음 |

두 NAT 배치 모두 기능적으로 가능하다. 다만 우리 코드는 `netfilter-persistent`를 활성화해 재부팅 후 규칙 복구까지 의도했고 실제로 `enabled`·`active`를 확인했다. 답지의 `nat_install.tpl`은 현재 실행 시점의 MASQUERADE 규칙만 추가하며 영속화 코드는 없다.

## 49. Web·AMI·ASG 차이

| 항목 | 우리 구현 | 강사 답지 |
|---|---|---|
| ASG Image | 조회한 Ubuntu Base AMI | Seed Web에서 생성한 Custom AMI |
| ASG `user_data` | 있음 | 없음 |
| Seed Web EC2 | 없음 | 1대 |
| AMI 준비 확인 | ALB Health Grace Period | Bastion `remote-exec`의 `curl` Loop |
| Seed 제거 | 해당 없음 | `aws ec2 terminate-instances` Local Exec |
| Seed Web Instance Profile | 해당 없음 | Local S3 Module의 Profile 연결 |
| ASG Instance Profile | S3 Read Profile 연결 | 없음. 필요한 Artifact는 Custom AMI에 포함 |
| ASG Security Group | Web SG 연결 | v2에서 Web SG 연결 |
| Target 등록 | ASG Traffic Source Attachment | ASG Traffic Source Attachment |

답지 v2는 `local_module/s3`가 만든 Instance Profile을 Seed Web EC2에 연결한다.

```text
module.s3.instance-profile-name
→ Seed Web EC2의 iam_instance_profile
→ web_install.tpl의 aws s3 cp
→ boot.war가 설치된 Custom AMI 생성
→ ASG Instance는 완성된 AMI로 시작
```

`web_install.tpl`은 Seed Web에서 다음 작업을 수행한다.

```bash
aws s3 cp "s3://${S3_BUCKET}/boot.war" ...
```

우리 방식은 ASG Instance가 부팅할 때마다 S3에서 Artifact를 받으므로 ASG Instance Profile이 필요하다. 답지 v2는 Seed Web이 한 번 Artifact를 받아 Custom AMI에 포함하므로 ASG Instance에 같은 S3 권한을 반복 부여하지 않아도 된다. 이는 **Image에 포함할 Build-time 의존성과 Instance가 실행 중 필요로 하는 Runtime 권한을 분리하는 예시**다.

## 50. Auto Scaling의 실제 의미
...
이는 Instance 2대를 유지하고 장애 시 교체하는 기능은 제공하지만, 부하에 따라 자동으로 2→4대로 늘리는 Scaling Policy는 아니다.
...
AWS 공식 문서에 따르면 Target Tracking은 지정 Metric을 목표값 근처로 유지하도록 Capacity를 자동 조정한다. CPU 평균 50%를 목표로 하면 부하가 높을 때 Scale-out하고 낮을 때 Scale-in한다.

이 부분은 답지에서 우리 구현으로 가져올 가치가 가장 높다.
...
강의 화면과 사용자 결정에서 Route 53·ACM을 제외했기 때문에 HTTPS와 DNS를 구현하지 않았다.
...
Route 53
→ www.kyes.click Alias
→ ALB
```

답지는 기능 범위를 HTTPS·DNS까지 넓혔다. 이 자체는 운영형 Web 진입점으로 배울 가치가 있다. 그러나 다음 사전 Resource가 해당 AWS 계정에 있어야 한다.

```text
ISSUED 상태의 kyes.click ACM Certificate
kyes.click Route 53 Hosted Zone
```

최초 답지 v1에는 `my-alb-logs`를 대상으로 한 ALB Access Log 설정이 있었지만 Bucket·Policy 생성 코드는 없었다. v2에서는 `access_logs` Block을 제거해 이 외부 선행 조건을 없앴다. 따라서 최신 답지의 확장 범위는 HTTPS·DNS이며 ALB Access Log는 아니다.

## 52. RDS 차이

| 항목 | 우리 구현 | 강사 답지 |
|---|---|---|
| Engine | MariaDB 11.8 | MariaDB 11.8 |
| Storage | gp3 20 GiB, 최대 40 GiB | 10 GiB, Type 미명시 |
| 암호화 | 명시적으로 활성화 | 코드에 명시 없음 |
| Multi-AZ | `multi_az = true` | `multi_az` 선언 없음 |
| Public 접근 | `false` 명시 | 코드에 명시 없음 |
| DB Subnet Group | VPC Module Output 사용 | RDS Module이 Subnet ID로 생성 |
| Password | `password_wo` | `password_wo` |
| 실제 검증 | `available`, Multi-AZ·비공개·암호화 확인 | 미수행 |

답지에는 Database Subnet 두 개가 있지만, Subnet 두 개와 Multi-AZ RDS는 같은 개념이 아니다. `multi_az` Input이 없으므로 답지 Source만으로는 Standby Replica 구성을 요구했다고 볼 수 없다.

AWS 공식 문서상 Multi-AZ DB Instance는 다른 AZ에 동기 Standby를 유지하여 장애 조치와 고가용성을 제공한다. 이는 Read Scaling 구성이 아니다. 우리 구현은 사용자가 설계 화면을 Multi-AZ로 해석한 결정을 코드와 Runtime에서 모두 확인했다.
...
```text
Registry S3 Bucket Module
→ Public Access 차단
→ TLS 강제 Policy
→ AES256 암호화

Registry S3 Object Submodule
→ boot.war Upload

Registry IAM Role Module
→ 정확한 boot.war ARN에 s3:GetObject
→ ASG Instance Profile로 연결
```
...
답지는 Local Module 구현을 복습할 수 있다는 장점이 있다. 반면 IAM Policy는 다음처럼 전체 Resource를 허용한다.

```hcl
Resource = "*"
```

v2에서는 생성한 Instance Profile Output을 Seed Web EC2가 소비한다. v1에서 끊겨 있던 `Output → Input` 연결이 v2에서 완성된 사례이므로, **Output을 공개한 뒤 실제 소비자 Input까지 연결해야 Architecture가 작동한다**는 학습 근거가 된다.

## 54. Security·Version·환경 의존성 차이

### Security Group

답지는 다음 Ingress를 `0.0.0.0/0`으로 연다.
...
우리 구현은 역할별 SG 참조로 좁혔다.

```text
관리자 /32 → NAT·Bastion SSH
Bastion SG → Web SSH
ALB SG → Web 8080
Web SG → RDS 3306
```
...
답지는 Registry Module `source`만 선언하고 `version`을 생략했다. HashiCorp 공식 문서는 Registry Module의 허용 Version을 명시적으로 제한하여 예기치 않은 변경을 피하라고 권장한다.

우리 구현은 다음 Version을 고정하고 실제 `.terraform.lock.hcl`·Init 상태와 맞췄다.

```text
VPC 6.6.1
EC2 6.4.0
Security Group 6.0.0
S3 5.14.1
IAM 6.6.1
RDS 7.2.0
ALB 10.5.0
...
my-public-ec2-key
my-private-ec2-key
kyes.click ACM·Hosted Zone
```

우리 구현은 Region·Profile·Key Pair·Instance Type·ASG Capacity·DB 값을 Variable로 두고 AMI·현재 Public IP·Key Pair를 Data Source로 확인한다. 다만 우리도 Lab Password가 State와 User Data에 포함되는 한계는 남아 있다.
...
| v1 관찰 | v2 수정 | 현재 판정 |
|---|---|---|
| Bastion Instance ID를 SSH Host에 전달 | `module.bastion_instance.public_ip` 사용 | 해소 |
| S3 Instance Profile Output 미사용 | Seed Web `iam_instance_profile`에 연결 | 해소 |
| 고정 `target_id`와 ASG Attachment 혼재 | 고정 `target_id` 제거 | 해소 |
| ASG Security Group 미연결 | Web SG 연결 | 해소 |
| 잘못된 RDS Parameter Family 표현 | `mariadb11.8`로 변경 | 해소 |

최신 v2의 연결은 이름과 실제 값의 의미가 일치한다.

```hcl
bastion-public-ip = module.bastion_instance.public_ip

iam_instance_profile = module.s3.instance-profile-name

security_groups = [module.web_security_group.id]
```

### 55-2. v2에도 남은 반복 Apply 수렴성 문제

답지의 AMI 이름은 다음과 같다.

```hcl
name = "${var.env}-boot-ami-${formatdate("YYYYMMDDhhmmss", timestamp())}"
```

HashiCorp 공식 문서는 `timestamp()` 결과가 매초 달라지므로 Resource Attribute에 직접 사용하면 Terraform 실행마다 Diff가 발생한다고 설명한다. 현재 AMI 흐름은 명시적 Release Version이나 Source Hash로 교체하는 편이 안정적이다.

또한 Terraform이 관리하는 `web_instance`를 `local-exec` AWS CLI로 종료하면 Configuration과 실제 상태가 분리된다. HashiCorp는 Provisioner의 행동을 Terraform이 예측 가능하게 모델링할 수 없으므로 목적별 도구를 먼저 고려하고 Provisioner는 마지막 수단으로 사용하라고 권고한다.

이 두 항목은 v2에서도 그대로다.

```text
timestamp() 기반 AMI Name
Terraform 관리 Seed EC2를 AWS CLI로 강제 종료
```

### 55-3. v2의 정리·환경 의존 확인 지점

| 위치 | 직접 관찰 | 판정 |
|---|---|---|
| Root ALB `name`·Tag | `${var.env}--alb` | 동작 가능성은 높지만 이중 하이픈은 오타로 보임 |
| AMI Module Input | `private-subnets`, `web-sg-id`를 받지만 내부에서 사용하지 않음 | 기능 오류보다는 불필요한 Interface |
| AMI Module SSH Key | `file("~/.ssh/my-public-ec2-key.pem")` | 실행 Host의 Home·Key 경로에 의존 |
| Module `source` | Version 제약 없음 | 최신 Module 변화에 따라 재현성이 달라질 수 있음 |

HashiCorp는 `~`로 시작하는 경로를 Home Directory로 바꿀 때 `pathexpand()`를 제공한다. 의도를 명시하려면 다음 형태가 더 분명하다.

```hcl
private_key = file(pathexpand("~/.ssh/my-public-ec2-key.pem"))
```

### 55-4. 외부 선행 조건

다음 항목은 코드 오류로 단정할 수 없지만 강사 계정에 이미 존재해야 한다.

```text
terra-user Profile
my-public-ec2-key
my-private-ec2-key
고정 AMI ID가 서울 Region에서 유효
kyes.click ACM Certificate
kyes.click Hosted Zone
```

우리 계정에서는 이전 실습에서 `my-public-ec2-key`가 없어 `InvalidKeyPair.NotFound`를 경험했다. 따라서 답지는 개인 계정에 그대로 Apply하는 파일이 아니라 환경값을 치환해야 하는 수업용 기준안으로 보는 것이 맞다.

### 55-5. 아직 판정하지 않은 항목

답지 폴더에서는 다음을 실행하지 않았다.
...
최초 답지 v1은 `terraform fmt -check -recursive`를 통과했다. 최신 v2는 `main.tf`의 Format 차이로 `fmt -check`를 통과하지 않았다. 이는 의미 오류를 입증하지는 않지만, 현재 Source가 표준 Format으로 정리되지 않았다는 뜻이다. Provider·Module Version에 따른 Argument 호환성과 AWS 생성 성공도 여전히 미확정이다.
...
우리 방식의 느린 Scale-out과 외부 의존성을 줄일 수 있는 방향이다. 단, 실무에서는 AMI Release를 Terraform Apply 안의 `remote-exec`·`local-exec`로 억지로 묶기보다 Packer나 EC2 Image Builder 같은 Image Pipeline을 별도로 고려하는 편이 낫다.

### 2. ASG의 `max_size`와 Scaling Policy는 다르다
...
→ 실제 부하에 따라 Desired Capacity를 조정하는 동작 규칙
```

우리 v17은 고가용성과 장애 교체를 검증했지만 부하 기반 Scale-out은 구현하지 않았다.

### 3. HTTPS와 DNS를 ALB Module에서 함께 조립

ACM Data Source, HTTPS Listener, HTTP Redirect, Route 53 Alias의 연결 흐름을 보여준다. 이번 사용자 확정 범위에서는 제외했지만 운영형 Web 진입점을 만들 때 재사용할 수 있다.

### 4. External Module과 Local Module의 혼합

모든 것을 External Module로 바꿀 필요는 없다. 조직 또는 실습에 고유한 AMI Build Workflow처럼 공개 Module로 그대로 표현하기 어려운 부분은 Local Module로 캡슐화할 수 있다.

### 5. Output은 공개한 뒤 소비자 Input에 연결해야 한다

답지 v1은 Instance Profile Output을 공개하고도 소비하지 않았지만, v2는 Seed Web의 `iam_instance_profile` Input에 연결했다. 이 변경은 Output 선언과 실제 데이터 흐름 연결의 차이를 직접 보여준다.

## 57. 무엇을 배우고 무엇을 채택할 것인가

### 바로 채택할 가치가 높은 것

```text
ASG CPU Target Tracking Policy
HTTPS Listener와 HTTP Redirect의 연결 원리
Custom AMI·Golden Image의 개념
Local Module로 고유 Pipeline을 감싸는 방식
Output을 실제 소비자 Input까지 연결하는 방식
`${var.env}` Prefix로 환경별 Resource 이름을 구분하는 방식
```

### 개념은 채택하되 현재 코드는 그대로 쓰지 않을 것

```text
Seed Web → Custom AMI
→ 학습 가치는 높음
→ 현재 Provisioner 구현 대신 별도 Image Pipeline 설계 필요
```

### 우리 구현을 유지할 것

```text
Registry Module Version 고정
관리자 /32·SG Reference 기반 접근 통제
명시적 DB Route 격리
RDS Multi-AZ·암호화·비공개 설정
S3 Object 한정 IAM Policy
ASG Instance Profile 연결
NAT Rule 영속화
Apply → Runtime → No changes → Destroy 검증 방식
```
...
Resource="*" S3 Policy
timestamp() 기반 AMI 교체
Terraform 관리 EC2의 AWS CLI 강제 종료
Module Version 미고정
환경별 ARN·AMI·Key·Domain 하드코딩
```

## 58. 최종 선택: 두 방식 중 어느 쪽이 더 적합한가

| 판단 기준 | 더 적합한 방식 | 이유 |
|---|---|---|
| 이번 v17 과제 완성 | 우리 방식 | 실제 전체 수명주기 검증 완료 |
| 빠른 ASG Scale-out | 답지의 Custom AMI 개념 | 부팅 시 설치 작업 감소 |
| Terraform Apply 단순성 | 우리 방식 | Seed·AMI·Provisioner 없음 |
| 반복 실행 수렴성 | 우리 방식 | 재실행 `No changes` 확인 |
| Golden Image 학습 | 답지 방식 | AMI Build 흐름을 직접 보여줌 |
| 보안 경계 | 우리 방식 | SG·IAM·DB Route가 더 좁음 |
| 실제 부하 기반 Scaling | 답지 | CPU Target Tracking 포함 |
| 운영형 HTTPS·DNS | 답지 | ACM·Route 53 연결 포함 |

최종 결론은 한쪽을 정답으로 선택하는 것이 아니다.

```text
우리 v17
→ 현재 요구사항을 안전하고 수렴 가능하게 완성한 실행본

강사 답지
→ Custom AMI·Target Tracking·HTTPS라는 다음 학습 주제를 포함한 확장안
```

답지 v2의 **개념적 확장과 연결 수정**은 배우되, 남아 있는 Image 수명주기 문제와 보안 완화까지 그대로 복사하지 않는 것이 적절하다.

## 59. v17.1 완료 판정

| 범위 | 상태 | 근거 |
|---|---|---|
| 파일 구조 비교 | 완료 | 두 폴더의 Source 목록·Block 개수 확인 |
| Artifact 비교 | 완료 | `boot.war` SHA-256 동일 |
| 전체 흐름 비교 | 완료 | Boot-time Provisioning과 Custom AMI 흐름 분리 |
| Module·Resource 소유권 비교 | 완료 | Registry·Local·Root 역할표 작성 |
| Network·SG·RDS·S3·ALB·ASG 비교 | 완료 | 실제 HCL Input·Output 기준 |
| 답지 v1 HCL Format | 완료 | `terraform fmt -check -recursive` 통과 |
| 답지 v2 HCL Format | 보완 필요 | `main.tf` Format 차이로 `fmt -check` 미통과 |
| 공식 문서 교차검증 | 완료 | Provisioner·timestamp·Module Version·Target Tracking·ALB Log·RDS Multi-AZ |
| 답지 Validate·Plan·Apply | 미수행 | 환경별 Profile·Key·Domain·Bucket 전제 존재 |
| 우리 v17 Runtime | 완료 | Part 6의 Apply·HTTP 200·NAT·RDS·Destroy 증거 |

v17.1은 **우리 구현을 답지에 맞게 덮어쓴 버전이 아니라, 두 구현의 의도와 데이터 흐름을 분리하고 학습 가치와 위험을 판정한 비교 노트**다.

## 60. v17.1 한 문단 요약

강사 최신 답지 v2와 우리 v17은 2-AZ VPC, NAT Instance, Bastion, Private Application, RDS, S3, ALB, ASG라는 큰 Architecture를 공유한다. 가장 큰 차이는 우리 구현이 ASG의 Launch Template에서 Base Ubuntu와 `user_data`를 사용해 Instance마다 Application을 구성하는 반면, 답지는 Seed Web EC2를 먼저 완성하고 Custom AMI를 만든 뒤 ASG가 그 Image를 사용한다는 점이다. v2는 최초 답지의 Bastion Host 값, Seed Web Instance Profile, ASG Security Group, 고정 Target ID, RDS Parameter Family 문제를 수정했다. 새로 배울 핵심은 Golden Image 흐름, Build-time S3 권한, CPU 50% Target Tracking, ACM·HTTPS·Route 53 조립이다. 그러나 `timestamp()`로 AMI 이름을 바꾸고 Terraform 관리 EC2를 AWS CLI로 종료하는 흐름은 반복 Apply의 수렴성을 해칠 수 있다. 우리 구현은 Module Version 고정, 좁은 SG·IAM, 명시적 DB 격리, Multi-AZ RDS, NAT 영속화와 실제 Apply·Runtime·Destroy 증거에서 우세하다. 따라서 v2의 연결 개선과 확장 개념은 학습하되 현재 v17 실행본은 유지하고, 향후 Custom AMI가 필요할 때는 별도 Image Pipeline으로 재설계하는 것이 최종 선택이다.
...
| 우리 구현 | 강사 답지 | 비고 |
|---|---|---|
| `versions.tf` | Root `main.tf`의 `terraform` Block | 우리는 CLI·Provider 조건 분리 |
| `variables.tf` | `variables.tf` | 우리는 환경·용량·Key·DB Input 확대 |
| `main.tf` | `main.tf` | Root 조립 코드 |
| `security_groups.tf` | Root `main.tf` SG Module | 우리는 SG를 별도 파일로 분리 |
| `templates/nat_install.sh` | `nat_install.tpl` | 우리 Script는 Rule 영속화 포함 |
| `templates/web_install.sh` | `web_install.tpl` | 우리 것은 ASG Boot-time, 답지는 Seed Web용 |
| `boot.war` | `local_module/s3/boot.war` | SHA-256 동일 |
| Registry S3·IAM Module | `local_module/s3` | External과 Local 구현 차이 |
| 없음 | `local_module/ami` | 답지만 Custom AMI Pipeline 보유 |
| `outputs.tf` | Local Module Output 3개 + Root Output 2개 | 우리는 운영·검증용 Root Output 확대 |

## 부록 H. 공식 근거

- [HashiCorp Terraform Provisioners](https://developer.hashicorp.com/terraform/language/provisioners)
- [HashiCorp `timestamp()` Function](https://developer.hashicorp.com/terraform/language/functions/timestamp)
- [HashiCorp `pathexpand()` Function 목록](https://developer.hashicorp.com/terraform/language/functions)
- [HashiCorp Module Block Reference](https://developer.hashicorp.com/terraform/language/block/module)
- [HashiCorp Registry Module 사용](https://developer.hashicorp.com/terraform/registry/modules/use)
- [AWS EC2 Auto Scaling Target Tracking](https://docs.aws.amazon.com/autoscaling/ec2/userguide/as-scaling-target-tracking.html)
- [AWS ALB Access Log 활성화](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/enable-access-logging.html)
...
- `D:\terraform\workspace\17_external_module` 실제 Source와 v17.0 Runtime Log
- `D:\terraform\workspace\17_external_module(답지)` 최초 답지 v1 Source
- `D:\terraform\workspace\17_external_module(답지v2)` 최신 답지 v2 Source
- `17_external_module(답지v2).zip`과 압축 해제 폴더의 파일별 SHA-256 대조
- 두 `boot.war` SHA-256 비교
- 두 폴더의 HCL Block·Input·Output·참조 검색
- 답지 v1·v2 `terraform fmt -check -recursive` 결과

### ② Authoritative External Evidence

- HashiCorp Terraform Provisioner·`timestamp()`·`pathexpand()`·Module Version 공식 문서
- AWS Target Tracking·ALB Access Log·RDS Multi-AZ 공식 문서

### ③ 수업 자료

- 강사가 제공한 `17_external_module(답지)`와 `17_external_module(답지v2)`
- 강의 화면의 2-AZ Architecture

### ④ 미검증 영역

- 답지의 `terraform init`·`validate`·`plan`·`apply`
- 답지의 Custom AMI 생성과 Seed Instance 종료 실제 동작
- 답지의 ACM·Route 53 외부 Resource 존재 여부
- 답지의 Module 최신 Version 호환성
