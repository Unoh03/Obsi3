...
source:
  ...
  - HashiCorp Terraform Provisioner·timestamp·Module Version 공식 문서
...
evidence:
  ...
  - 강사 답지와 우리 구현의 HCL Block·흐름·Artifact Hash 정적 대조
  - 강사 답지 terraform fmt -check -recursive 통과
...
v17.0에서는 강사의 설계 화면과 사용자 결정을 바탕으로 External Registry Module 중심의 Architecture를 독립적으로 구현했다. 이후 `17_external_module(답지)`가 공개됐으므로 v17.1에서는 두 구현을 다음 기준으로 대조한다.
...
| Root·Local `output` Block | 10개 | 3개 |
...
| ASG Instance Profile | S3 Read Profile 연결 | 연결 코드 없음 |
| Target 등록 | ASG Traffic Source Attachment | ASG Traffic Source Attachment |

답지의 Custom AMI 흐름은 좋은 학습 주제다. 그러나 현재 코드에서는 `local_module/s3`가 Instance Profile을 만들고 Output도 공개하지만 `web_instance`와 `asg` 어느 곳에도 전달하지 않는다.
...
동시에 `web_install.tpl`은 다음 작업을 요구한다.

```bash
aws s3 cp "s3://${S3_BUCKET}/boot.war" ...
```

따라서 Instance에 다른 AWS Credential이 없다면 S3 Download가 실패할 가능성이 높다. 우리 구현은 ASG에 `iam_instance_profile_name`을 명시적으로 전달했다.
...
## 51. ALB·HTTPS·DNS·Access Log 차이
...
my-alb-logs S3 Bucket와 ELB Log Delivery Policy
```

AWS 공식 문서상 ALB Access Log 대상 Bucket에는 ELB가 Object를 기록할 수 있는 Bucket Policy가 필요하다. 답지에는 `my-alb-logs` Bucket이나 Policy 생성 코드가 없으므로 외부 선행 조건이다.
...
또한 생성한 Instance Profile Output이 실제 EC2·ASG Input으로 연결되지 않는다. 따라서 **Output을 공개하는 것과 소비자가 실제로 사용하는 것은 별개**라는 좋은 반례가 된다.
...
### 55-1. 코드에서 직접 확인된 연결 문제

| 위치 | 관찰 | 영향 |
|---|---|---|
| Root `main.tf:411` | `bastion-public-ip = module.bastion_instance.id` | SSH `host`에 Instance ID가 전달됨 |
| `local_module/s3/output.tf` | Instance Profile Output 공개 | Root에서 소비하지 않음 |
| `web_install.tpl` | `aws s3 cp` 실행 | IAM Profile 미연결 시 Download 실패 가능 |
| `local_module/ami/main.tf:41` | AWS CLI로 Terraform 관리 EC2 종료 | 다음 Refresh에서 Drift·재생성 가능 |
| `main.tf:324` | 고정 `target_id` 존재 | `create_attachment=false`와 함께 있어 의미가 혼란스러움 |

`bastion-public-ip`에는 다음 값이 들어가는 것이 이름과 사용처에 맞다.
...
my-alb-logs Bucket와 Bucket Policy
```
...
`terraform fmt -check -recursive`는 통과했다. 따라서 HCL Format은 확인했지만 Provider·Module Version에 따른 Argument 호환성과 AWS 생성 성공은 미확정이다.
...
### 5. Output은 공개만으로 끝나지 않는다

답지의 Instance Profile처럼 Output을 선언해도 Root 또는 다른 Module이 소비하지 않으면 실제 Architecture에는 연결되지 않는다.
...
답지의 **개념적 확장**은 배우되, 확인된 연결 문제와 보안 완화까지 그대로 복사하지 않는 것이 적절하다.
...
| 답지 HCL Format | 완료 | `terraform fmt -check -recursive` 통과 |
...
강사 답지와 우리 v17은 2-AZ VPC, NAT Instance, Bastion, Private Application, RDS, S3, ALB, ASG라는 큰 Architecture를 공유한다. 가장 큰 차이는 우리 구현이 ASG의 Launch Template에서 Base Ubuntu와 `user_data`를 사용해 Instance마다 Application을 구성하는 반면, 답지는 Seed Web EC2를 먼저 완성하고 Custom AMI를 만든 뒤 ASG가 그 Image를 사용한다는 점이다. 답지에서 새로 배울 핵심은 Golden Image 흐름, CPU 50% Target Tracking, ACM·HTTPS·Route 53 조립이다. 그러나 Instance ID를 Bastion SSH Host로 전달하고, S3 Instance Profile을 소비하지 않으며, `timestamp()`로 AMI 이름을 바꾸고 Terraform 관리 EC2를 AWS CLI로 종료하는 부분은 그대로 채택하기 어렵다. 우리 구현은 Module Version 고정, 좁은 SG·IAM, 명시적 DB 격리, Multi-AZ RDS, NAT 영속화와 실제 Apply·Runtime·Destroy 증거에서 우세하다. 따라서 답지의 확장 개념은 학습하되 현재 v17 실행본은 유지하고, 향후 Custom AMI가 필요할 때는 별도 Image Pipeline으로 재설계하는 것이 최종 선택이다.
...
| `outputs.tf` | Local Module Output 3개 | 우리는 운영·검증용 Root Output 확대 |
...
- `D:\terraform\workspace\17_external_module(답지)` 실제 Source
- 두 `boot.war` SHA-256 비교
- 두 폴더의 HCL Block·Input·Output·참조 검색
- 답지 `terraform fmt -check -recursive` 결과
...
- 답지의 ACM·Route 53·ALB Access Log 외부 Resource 존재 여부
