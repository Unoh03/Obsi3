...
source:
  ...
  - 07_servers/main.tf
  - 08_quiz/main.tf
  - 08_quiz/nat_install.tpl
  - 08_quiz/web_install.tpl
  - 08_quiz/boot.war
  - 09_file_division/main.tf
  - 09_file_division/networks.tf
  - 09_file_division/instances.tf
  - 09_file_division/s3.tf
  - 09_file_division/load-balancer.tf
  - 09_file_division/variables.tf
  - 09_file_division/outputs.tf
  - 09_file_division/nat_install.tpl
  - 09_file_division/web_install.tpl
  - 09_file_division/boot.war
official_refs:
...
> **v9.0 부제:** 종합 웹 서비스 아키텍처와 Terraform 파일 분할

## 목적

이 노트는 기존 `v6.3`의 **Public Web + Private DB EC2** 구조를 RDS 기반 구조로 전환하고, 같은 인프라를 **Network와 Server라는 두 개의 독립적인 Terraform Root Module**로 분리한 뒤, 강사님의 `08_quiz`과 `09_file_division`을 통해 **종합 웹 서비스 아키텍처 조립과 하나의 Root Module 내부 파일 분할**까지 누적 정리한다.
...
v8.0:
VPC, EC2, RDS, S3, IAM, ALB, ACM, Route 53
→ 기존에 배운 요소를 하나의 웹 서비스 아키텍처로 통합
→ 사용자 요청 흐름과 애플리케이션 배포 흐름 연결

v9.0:
08_quiz의 단일 main.tf
→ 09_file_division의 역할별 .tf 파일로 분할
→ Root Module, State, Dependency Graph는 그대로 유지
→ 후속 반복문 진도를 이해하기 위한 구조적 준비
```
...
> [!note] 버전 진행 순서
> 실제 수업 진도상 Root Module과 State 분리 실습을 먼저 진행하게 되어, v7.1의 WEB-RDS 기능 검증을 보류한 상태에서 v7.2 내용을 먼저 추가했다.  
> 따라서 현재 본문은 v7.0 다음에 v7.2가 이어지며, v7.1은 실제 검증 결과가 확보되면 중간 Part로 보완한다.
...
> [!important] v9.0의 핵심
> `08_quiz`은 지금까지 배운 AWS 리소스를 하나의 서비스 흐름으로 조립한다.  
> `09_file_division`은 같은 구성을 여러 `.tf` 파일로 재배치하지만, **Root Module, State, Dependency Graph, 실행 단위는 나누지 않는다.**

> [!note] 증적 범위
> v7.2는 Terraform의 구성 경계와 State 연동 원리를 설명하는 실습 노트다.  
> 별도의 브라우저 스크린샷이나 고의적인 오류 증적은 필수로 요구하지 않는다. 코드 구조, 명령, 예상 동작을 중심으로 정리한다.

> [!note] v8.0·v9.0 기록 범위
> 이번 추가분은 강사 코드에서 직접 확인되는 구성과 교육 흐름을 정리한다.  
> 실제 `terraform apply`, ALB Health Check, HTTPS 접속, Route 53 전파 결과는 수행한 것으로 기록하지 않는다.
...
- 구성 분리의 장점과 운영 비용 비교
- ALB, Target Group, Listener, Route 53 Alias의 역할 해석
- IAM Role과 Instance Profile을 이용한 EC2 권한 전달 해석
- 하나의 Root Module 내부 파일 분할과 Dependency Graph 해석
...
- AWS EC2 IAM Role과 Instance Profile 공식 문서
- AWS Application Load Balancer Listener 공식 문서
- AWS Route 53에서 ELB로 트래픽을 라우팅하는 공식 문서
...
- 08_quiz/main.tf, nat_install.tpl, web_install.tpl, boot.war
- 09_file_division의 역할별 `.tf` 파일, 템플릿, boot.war
...
> [!abstract] 주요 이동
> - [[#Part 1. v7.0 RDS 인프라 구성과 시행착오|Part 1. v7.0 RDS 인프라 구성]]
> - [[#Part 2. v7.2 Network와 Server Root Module 분리|Part 2. Root Module 분리]]
> - [[#Part 3. v8.0 종합 웹 서비스 아키텍처|Part 3. 종합 웹 서비스 아키텍처]]
> - [[#Part 4. v9.0 Terraform 파일 분할|Part 4. Terraform 파일 분할]]
> - [[#20. `output`과 `terraform_remote_state`로 데이터 전달|State 데이터 전달]]
> - [[#22. State가 나뉘면 전체 의존성 그래프도 나뉜다|State와 생명주기]]
> - [[#26. 현재 `07_networks`와 `07_servers` 검토|현재 코드 검토]]
> - [[#27. v7.2 구현 순서|구현 순서]]
> - [[#28. v7.2 완료 판정|v7.2 완료 판정]]
> - [[#39. v8.0 학습 판정|v8.0 학습 판정]]
> - [[#49. v9.0 완료 판정|v9.0 완료 판정]]
> - [[#Appendices|부록]]

> [!note] 탐색 메모
> 긴 Terraform, Shell, 로그 블록은 접힘 callout으로 유지한다.  
> 세부 절 이동은 Obsidian 오른쪽 사이드바의 **Outline** core plugin을 사용한다.

---

# Part 1. v7.0 RDS 인프라 구성과 시행착오
...
# Part 2. v7.2 Network와 Server Root Module 분리

> [!warning] 실행 전 설계
> 이 Part의 `07_networks`와 `07_servers` 구조는 현재 코드 및 설계 기준으로 정리했다.  
> `terraform_remote_state`의 실제 output 조회, 두 Root Module의 apply/destroy 결과는 실행 후 확인 항목이며, 아직 관찰 완료로 간주하지 않는다.
...
# Part 3. v8.0 종합 웹 서비스 아키텍처

> [!note] 직접 관찰과 해석의 구분
> 이 Part에서 **코드에서 직접 관찰**로 표시한 내용은 `08_quiz`의 Terraform 및 템플릿에서 확인한 사실이다.  
> **학습 의도 해석**은 코드 구성과 실제 진도 순서를 바탕으로 해석한 것이며, 강사의 발언을 그대로 인용한 것은 아니다.

## 30. v8.0의 목적

### 30-1. 코드에서 직접 관찰

`08_quiz/main.tf`는 다음 구성요소를 하나의 Terraform Root Module에 함께 선언한다.

```text
Network:
VPC, Public/Private Subnet, Internet Gateway, Route Table, NAT Instance

Compute와 Database:
Bastion EC2, Private Web EC2, RDS MariaDB, Security Group

Application artifact와 권한:
S3 Bucket, boot.war Object, IAM Role, Inline Policy, Instance Profile

외부 접점:
Application Load Balancer, Target Group, Listener, ACM Certificate Data Source,
Route 53 Hosted Zone Data Source, Alias Record
```

### 30-2. 학습 의도 해석

이 단계의 핵심은 개별 Terraform 리소스 문법을 하나씩 추가하는 것보다, 앞서 배운 리소스를 실제 서비스 흐름으로 연결하는 데 있다.

```text
개별 리소스 생성
→ 리소스 사이의 참조 관계 구성
→ 애플리케이션 배포
→ 외부 도메인과 HTTPS 접속 지점 완성
```

즉 `08_quiz`라는 이름과 달리, 실제 내용은 지금까지의 AWS/Terraform 진도를 조립하는 **종합 웹 서비스 아키텍처 실습**에 가깝다.
...
| 계층 | 주요 리소스 | 역할 |
|---|---|---|
| DNS | Route 53 Hosted Zone, Alias Record | `www.kyes.click`을 ALB로 연결 |
| TLS/진입점 | ACM Certificate, ALB Listener | HTTPS 처리와 HTTP→HTTPS Redirect |
| 부하 분산 | ALB, Target Group, Attachment | 요청을 Web EC2의 8080 포트로 전달 |
| 애플리케이션 | Private Web EC2, Tomcat, `boot.war` | 웹 애플리케이션 실행 |
| Artifact | S3 Bucket, S3 Object | 배포할 WAR 파일 보관 |
| 권한 | IAM Role, Policy, Instance Profile | Web EC2가 S3 Object를 읽도록 권한 전달 |
| Database | RDS MariaDB, DB Subnet Group | 애플리케이션 데이터 저장 |
| 관리 경로 | Bastion EC2 | Public Subnet에서 Private 영역 접근의 중간 지점 |
| Outbound | NAT Instance, Private Route Table | Private Subnet의 외부 패키지 설치 경로 제공 |

### 31-2. Terraform 참조가 만드는 연결

`08_quiz`에서는 다음과 같은 참조 관계가 하나의 Dependency Graph를 만든다.
...
이 구조에서는 리소스를 생성하는 것만으로 충분하지 않다.

```text
어떤 리소스가 값을 제공하는가
→ 어떤 리소스가 그 값을 소비하는가
→ 그 참조가 생성 순서에 어떤 영향을 주는가
```
...
│  │  ├─ NAT Instance
│  │  └─ ALB node
│  ├─ Private Web Subnet 192.168.11.0/24
│  │  └─ Web EC2
│  └─ Private DB Subnet  192.168.12.0/24
│     └─ RDS DB Subnet Group member
│
└─ ap-northeast-2c
   ...
   │  ├─ Bastion EC2
   │  └─ ALB node
   ├─ Private Web Subnet 192.168.31.0/24
   └─ Private DB Subnet  192.168.32.0/24
      └─ RDS DB Subnet Group member
```

현재 Web EC2는 `ap-northeast-2a`의 Private Web Subnet에 1대만 선언되어 있다. 두 번째 Private Web Subnet은 구조에 존재하지만 Web EC2가 배치되지는 않는다.
...
→ HTTPS Listener :443
→ Target Group
→ Private Web EC2 :8080
→ Tomcat 애플리케이션
→ RDS MariaDB :3306
```

HTTP 요청은 별도 Listener에서 HTTPS로 전환된다.
...
→ S3 Bucket
→ Web EC2의 IAM Instance Profile
→ user_data에서 aws s3 cp
→ Tomcat webapps/boot.war
→ 애플리케이션 압축 해제
→ application.properties 수정
→ Tomcat 재시작
```

사용자 요청 흐름과 배포 흐름은 목적이 다르다.

| 흐름 | 목적 |
|---|---|
| 사용자 요청 흐름 | 외부 사용자의 DNS·HTTPS 요청을 애플리케이션과 DB까지 전달 |
| 애플리케이션 배포 흐름 | Web EC2가 S3에서 실행 파일을 받아 Tomcat에 배치 |

---

## 33. S3와 IAM Role 기반 애플리케이션 배포

### 33-1. 코드에서 직접 관찰

Terraform은 로컬의 `boot.war`를 S3 Object로 업로드한다.

```hcl
resource "aws_s3_object" "quiz-boot-object" {
  bucket = aws_s3_bucket.quiz-s3-bucket.id
  key    = "boot.war"
...
Web EC2에는 IAM Instance Profile이 연결된다.

```hcl
iam_instance_profile = aws_iam_instance_profile.quiz-instance-profile.name
```

`web_install.tpl`은 AWS CLI로 S3 Object를 다운로드한다.

```bash
aws s3 cp "s3://${S3_BUCKET}/boot.war" /opt/tomcat/tomcat-10/webapps/boot.war
```

### 33-2. Role, Policy, Instance Profile 관계
...
IAM Role Policy:
Role이 실행할 수 있는 AWS API 동작 정의

Instance Profile:
IAM Role을 EC2 Instance에 연결하는 컨테이너

Web EC2:
iam_instance_profile 속성으로 Instance Profile 사용
```
...
이 구성의 학습 포인트는 Access Key를 `user_data`에 직접 넣지 않고, EC2에 역할을 부여해 S3 API를 호출하도록 만드는 것이다.

AWS 공식 문서에서도 EC2 애플리케이션이 장기 자격증명을 직접 관리하는 대신 IAM Role과 임시 자격증명을 사용하도록 설명하며, EC2에서는 Instance Profile이 Role의 컨테이너 역할을 한다.

### 33-4. 이번 노트의 범위

현재 Policy의 `Resource = "*"`가 최소 권한인지 평가하고 수정하는 작업은 이번 범위에서 제외한다. 여기서는 **Role → Policy → Instance Profile → EC2** 연결 원리를 학습 대상으로 본다.

---

## 34. ALB, Target Group, Listener

### 34-1. Application Load Balancer

```hcl
resource "aws_lb" "quiz-alb" {
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.quiz-alb-sg.id]
  subnets = [
    aws_subnet.quiz-public-subnet-2a.id,
    aws_subnet.quiz-public-subnet-2c.id
  ]
}
```
...
```text
internet-facing ALB
→ 서로 다른 2개 AZ의 Public Subnet 사용
→ 외부 HTTP/HTTPS 요청의 진입점
```

### 34-2. Target Group

```hcl
resource "aws_lb_target_group" "quiz-alb-tg" {
  port        = 8080
  protocol    = "HTTP"
...
Target Group은 다음을 정의한다.

```text
- 어느 VPC의 대상을 사용할지
- 어떤 프로토콜과 포트로 전달할지
- 대상의 정상 여부를 어떤 경로로 확인할지
- 대상 유형이 Instance인지 IP인지
```

### 34-3. Attachment

```hcl
resource "aws_lb_target_group_attachment" "quiz-lb-web-attachment" {
  target_group_arn = aws_lb_target_group.quiz-alb-tg.arn
  target_id        = aws_instance.quiz-web-instance.id
  port             = 8080
}
```
...
Target Group 정의
+ Web EC2 등록
= ALB가 실제 요청을 보낼 대상 완성
```

### 34-4. Listener

AWS 공식 문서 기준 Listener는 지정된 프로토콜과 포트에서 연결 요청을 확인하고, 규칙에 따라 등록된 대상으로 요청을 전달한다.

```text
ALB:
요청을 받는 Load Balancer 본체

Listener:
어떤 포트와 프로토콜에서 어떤 Action을 수행할지 결정

Target Group:
실제 전달 대상과 Health Check 기준 정의

Attachment:
Target Group에 Web EC2 등록
```
...
```hcl
data "aws_acm_certificate" "quiz-acm" {
  domain   = "kyes.click"
  statuses = ["ISSUED"]
}
```

이 블록은 인증서를 새로 생성하지 않는다.

```text
resource:
Terraform이 생성·관리할 대상

data source:
이미 존재하는 AWS 대상을 조회
```

`data.aws_acm_certificate.quiz-acm.arn`은 HTTPS Listener에 전달된다.
...
→ ALB가 ACM 인증서 사용
→ TLS 처리
→ Target Group으로 HTTP :8080 전달
```

AWS 공식 문서에 따르면 ALB의 HTTPS Listener에는 최소 하나의 서버 인증서가 필요하며, ALB가 암호화·복호화 작업을 처리할 수 있다.
...
```text
HTTP :80 요청
→ 애플리케이션으로 직접 Forward하지 않음
→ HTTPS :443으로 Redirect
```

### 35-4. 학습 의도 해석

이 단계에서는 인증서 파일을 Web EC2에 직접 배치하는 대신, 외부 TLS 종단을 ALB에 맡기는 구조를 학습한다.
...
이것은 현재 코드에서 확인되는 통신 구조이며, Web EC2까지 HTTPS를 유지하는 End-to-End TLS 구성은 아니다.

---

## 36. Route 53 Alias와 ALB

### 36-1. Hosted Zone 조회

```hcl
data "aws_route53_zone" "quiz-route53-zone" {
  name = "kyes.click"
}
```

이 Data Source는 이미 존재하는 Hosted Zone을 조회한다.

### 36-2. Alias Record 생성

```hcl
resource "aws_route53_record" "quiz-www" {
  zone_id = data.aws_route53_zone.quiz-route53-zone.id
  name    = "www.${data.aws_route53_zone.quiz-route53-zone.name}"
  type    = "A"

  alias {
    name                   = "dualstack.${aws_lb.quiz-alb.dns_name}"
    zone_id                = aws_lb.quiz-alb.zone_id
...
AWS 공식 문서는 Route 53에서 ELB Load Balancer로 도메인 트래픽을 보낼 때 Alias Record를 사용하도록 설명한다. Alias는 CNAME과 유사하지만 루트 도메인과 하위 도메인 모두에서 AWS 대상 리소스를 가리킬 수 있다.
...
Route 53 Alias:
사용자가 사용할 도메인 이름을 ALB와 연결
```

Route 53는 웹 요청을 직접 처리하지 않는다. DNS 질의에 대해 ALB로 연결되는 이름 해석 결과를 제공한다.
...
| 구성요소 | 이번 코드에서의 역할 |
|---|---|
| `aws_s3_object` | 로컬 `boot.war`를 S3 Artifact로 업로드 |
| `aws_iam_role` | EC2가 맡을 수 있는 권한 주체 정의 |
| `aws_iam_role_policy` | S3 Object 조회 권한 정의 |
| `aws_iam_instance_profile` | IAM Role을 Web EC2에 연결 |
| `aws_lb` | 외부 HTTP/HTTPS 진입점 |
| `aws_lb_target_group` | Web EC2 전달 포트와 Health Check 정의 |
| `aws_lb_listener` | 80 Redirect, 443 Forward 동작 정의 |
| `aws_lb_target_group_attachment` | Web EC2를 Target Group에 등록 |
| `data.aws_acm_certificate` | 기존 발급 인증서 조회 |
| `data.aws_route53_zone` | 기존 Hosted Zone 조회 |
| `aws_route53_record` | `www` Alias를 ALB에 연결 |

### 37-2. 기존 개념과의 연결

```text
Resource와 Data Source
→ ACM/Hosted Zone은 조회, Alias/ALB는 생성

Implicit Dependency
→ 리소스 속성 참조가 생성 순서를 형성

Templatefile
→ RDS Endpoint, DB 계정, S3 Bucket을 user_data에 전달

IAM
→ 애플리케이션 자격증명을 코드에 직접 저장하지 않고 Role로 전달
```
...
[x] `08_quiz`의 구성요소 식별
[x] 사용자 요청 흐름 해석
[x] S3 Artifact 배포 흐름 해석
[x] IAM Role / Policy / Instance Profile 관계 정리
[x] ALB / Target Group / Listener 역할 구분
[x] ACM Data Source와 HTTPS Listener 연결 정리
[x] Route 53 Alias와 ALB 연결 정리
[x] Resource와 Data Source 관계 확인
```

### 38-2. 이번 Part에 포함하지 않음

```text
[ ] 실제 terraform init / validate / plan / apply
[ ] ALB Target Health 확인
[ ] 브라우저에서 HTTP → HTTPS Redirect 확인
[ ] ACM 인증서와 실제 도메인 일치 여부 확인
[ ] Route 53 DNS 전파 확인
[ ] boot.war 애플리케이션 실행 확인
[ ] RDS Schema 생성 결과 확인
[ ] 강사 계정 종속값을 사용자 계정에 맞게 수정
[ ] Security Group과 IAM Policy 최소 권한화
```

### 38-3. 코드 해석과 실행 검증의 경계

```text
현재 확인한 것:
Terraform 코드와 템플릿이 의도하는 구조

아직 확인하지 않은 것:
현재 AWS 계정과 Provider 버전에서 실제로 생성·동작하는지
```
...
```text
[x] 기존 VPC, EC2, RDS 실습을 종합 웹 서비스 흐름으로 연결
[x] Artifact, 권한, 부하 분산, TLS, DNS 계층 추가
[x] 사용자 요청 흐름과 애플리케이션 배포 흐름 구분
```

### 39-2. 코드 구조 확인

```text
[x] 08_quiz/main.tf의 Terraform 블록 확인
[x] 44개 최상위 HCL 블록 식별
[x] nat_install.tpl 확인
[x] web_install.tpl 확인
[x] boot.war가 S3 Object source로 사용됨을 확인
```
...
```text
v8.0은 실행 완료 기록이 아니라,
강사 코드가 여러 AWS 서비스를 하나의 웹 서비스 아키텍처로
어떻게 연결하는지 파악한 단계로 판정한다.
```
...
```text
v8.0에서는 기존에 개별적으로 학습한 VPC, Subnet, NAT Instance, Bastion, Web EC2, RDS를 하나의 Root Module에 통합하고, S3와 IAM Instance Profile을 이용한 애플리케이션 배포, Application Load Balancer와 Target Group을 이용한 요청 분산, ACM 인증서를 사용하는 HTTPS Listener, Route 53 Alias를 통한 사용자 도메인 연결을 추가하였다. 외부 사용자의 요청은 Route 53과 ALB를 거쳐 Private Web EC2의 Tomcat 애플리케이션으로 전달되고, 애플리케이션은 RDS MariaDB를 사용한다. 별도로 Web EC2는 IAM Role의 임시 권한을 통해 S3에서 `boot.war`를 내려받아 배포한다. 이번 단계에서는 코드가 의도하는 전체 연결 관계를 파악했으며, 실제 apply와 DNS·HTTPS·Health Check 검증은 수행 완료로 기록하지 않는다.
```

---

# Part 4. v9.0 Terraform 파일 분할

> [!note] 교육 흐름 해석
> `09_file_division` 다음에 반복문 진도가 이어진다는 실제 수업 순서를 근거로, 이번 파일 분할은 반복되는 리소스 구조를 식별하기 위한 준비 단계로 해석한다.  
> 이는 코드 구조와 진도 순서를 바탕으로 한 해석이며, 강사가 명시한 문장을 그대로 옮긴 것은 아니다.

## 41. v9.0의 목적

### 41-1. 직접 관찰

`08_quiz`은 Terraform 구성을 하나의 `main.tf`에 모아 두었다.
...
`09_file_division`은 같은 최상위 HCL 블록을 역할별 `.tf` 파일로 분산했다.
...
```text
파일을 나누면 Terraform 구성도 여러 개가 되는가?

파일 사이에서 다른 리소스를 직접 참조할 수 있는가?

파일 이름과 배치 순서가 리소스 생성 순서를 결정하는가?

v7.2의 폴더 분리와 이번 파일 분할은 무엇이 다른가?
```
...
`08_quiz/main.tf`는 446줄이며 다음 종류의 블록을 한 파일에 함께 둔다.

```text
terraform / provider
network
NAT / EC2 / RDS / Security Group
S3 / IAM
variable / output
ALB / ACM / Route 53
```

### 42-2. 학습 및 유지보수 관점의 문제

```text
- 원하는 리소스를 찾기 위해 긴 파일을 탐색해야 함
- 네트워크, 서버, Artifact, 외부 접점의 책임 경계가 흐려짐
- 반복되는 Subnet과 Association 구조를 한눈에 비교하기 어려움
- 여러 사람이 수정할 때 같은 파일에서 충돌하기 쉬움
- 특정 계층만 검토하려 해도 다른 계층의 코드가 함께 보임
```

### 42-3. 주의

파일이 길다는 이유만으로 State를 나눌 필요는 없다.

```text
코드 탐색이 불편함:
같은 Root Module에서 .tf 파일 분할 검토

배포 주기·권한·생명주기가 다름:
Root Module과 State 분리 검토
```

이번 `09_file_division`은 첫 번째 문제를 해결하는 단계다.
...
| 파일 | 최상위 블록 수 | 책임 |
|---|---:|---|
| `main.tf` | 2 | Terraform Provider 요구사항과 AWS Provider 설정 |
| `networks.tf` | 18 | VPC, 6개 Subnet, IGW, Route Table, Association, NAT Instance와 NAT SG |
| `instances.tf` | 7 | Bastion, Web, RDS, Server Security Group, DB Subnet Group |
| `s3.tf` | 5 | S3 Bucket/Object, IAM Role/Policy, Instance Profile |
| `load-balancer.tf` | 9 | ALB SG, ALB, Target Group, ACM, Listener, Attachment, Route 53 |
| `variables.tf` | 2 | RDS 사용자명과 비밀번호 입력 변수 |
| `outputs.tf` | 1 | Bastion Public IP 출력 |
| **합계** | **44** | `08_quiz/main.tf`의 44개 최상위 블록과 동일 |
...
instances.tf:
Compute, Database, workload 통신 정책

s3.tf:
Artifact 저장과 Web EC2 권한

load-balancer.tf:
외부 요청 진입점, TLS, DNS

variables.tf / outputs.tf:
Root Module의 입력과 출력 인터페이스
```

### 43-3. 파일명은 관례다

Terraform이 반드시 `main.tf`, `variables.tf`, `outputs.tf`라는 이름을 요구하는 것은 아니다. 같은 디렉터리의 최상위 `.tf` 파일을 모두 평가하므로, 이 이름은 사람에게 책임을 드러내기 위한 관례다.

---

## 44. 하나의 Root Module과 State

### 44-1. HashiCorp 공식 문서 기준

Terraform Module은 한 디렉터리에 함께 있는 최상위 `.tf` 또는 `.tf.json` 파일의 모음이다. Terraform은 같은 Module의 모든 구성 파일을 하나의 문서처럼 평가하며, 여러 파일로 나누는 것은 독자와 유지보수자의 편의를 위한 것이다.

### 44-2. 현재 구조

```text
09_file_division 디렉터리
├─ main.tf
├─ networks.tf
├─ instances.tf
├─ s3.tf
...
```text
09_file_division 안의 여러 .tf:
같은 구성의 파일 정리

07_networks와 07_servers:
서로 다른 작업 디렉터리와 Root Module
```

`09_file_division`에서는 `terraform_remote_state`가 필요하지 않다. 모든 리소스 주소가 같은 Module의 가시 범위에 있기 때문이다.

---

## 45. 파일 사이의 참조와 Dependency Graph

### 45-1. Network → Instance

`instances.tf`의 Web EC2는 `networks.tf`의 Subnet을 직접 참조한다.

```hcl
subnet_id = aws_subnet.quiz-private-web-subnet-2a.id
```

### 45-2. S3/IAM → Instance

`instances.tf`의 Web EC2는 `s3.tf`의 Instance Profile과 S3 Bucket을 참조한다.
...
```hcl
S3_BUCKET = aws_s3_bucket.quiz-s3-bucket.id
```

### 45-3. RDS → Instance user_data

```hcl
RDS_ENDPOINT = aws_db_instance.quiz-rds-instance.address
```

이 참조 때문에 Web EC2의 `user_data` 렌더링은 RDS Endpoint 값을 필요로 한다.

### 45-4. Network/Instance → Load Balancer

`load-balancer.tf`는 다른 파일의 VPC, Public Subnet, Web EC2를 직접 참조한다.
...
참조 관계:
Terraform이 Dependency Graph를 만드는 경계
```

파일이 달라도 같은 Module 안에서는 리소스 주소를 직접 참조한다.
...
Terraform은 위와 같은 파일 실행 순서를 사용하지 않는다.

### 46-2. 실제 해석

```text
같은 디렉터리의 모든 .tf 파일 로드
→ 하나의 구성으로 결합
→ Expression Reference 분석
→ Dependency Graph 생성
→ Graph에 따라 실행 순서 결정
```
...
```text
ALB가 Public Subnet ID 참조
→ Subnet이 먼저 필요

Target Group Attachment가 Web EC2 ID 참조
→ Web EC2와 Target Group이 먼저 필요

HTTPS Listener가 ACM ARN 참조
→ 인증서 조회 결과가 먼저 필요
```

### 46-3. 숫자 접두사도 순서를 강제하지 않는다
...
처럼 이름을 바꿔도 사람이 보는 정렬 순서만 달라진다. 리소스 생성 순서는 참조 관계와 명시적 `depends_on`으로 결정된다.

---

## 47. v7.2 Root Module 분리와 비교

| 항목 | v7.2 | v9.0 |
|---|---|---|
| 구조 | `07_networks`, `07_servers` | `09_file_division` 하나 |
| 디렉터리 | 2개 | 1개 |
| Root Module | 2개 | 1개 |
| State | 2개 | 1개 |
| Dependency Graph | 2개 | 1개 |
| 다른 영역 리소스 직접 참조 | 불가 | 가능 |
| 데이터 전달 | `output` + `terraform_remote_state` | 리소스 주소 직접 참조 |
| `init / plan / apply` | 각 디렉터리에서 별도 실행 | 한 디렉터리에서 한 번 실행 |
| 생성·삭제 순서 | Root Module 사이 순서를 사용자가 관리 | 하나의 Graph에서 Terraform이 관리 |
| 주목적 | 생명주기·책임·State 경계 분리 | 가독성과 유지보수성 개선 |

### 47-1. 같은 “분리”지만 결과가 다르다

```text
v7.2 폴더 분리:
Terraform이 보는 구성 경계 자체가 바뀜

v9.0 파일 분리:
Terraform이 보는 구성은 그대로이고 사람이 읽는 구조만 바뀜
```
...
Network와 Server의 변경 주기·권한·생명주기가 다름:
Root Module과 State 분리 검토
```
...
`networks.tf`에는 값만 달라지는 유사한 블록이 반복된다.
...
Security Group에도 유사한 중첩 블록이 반복된다.
...
즉 `09_file_division` 자체는 반복문 실습이 아니다.

### 48-3. 교육 흐름 해석

파일을 역할별로 나누면 반복되는 네트워크 리소스가 `networks.tf`에 모이고, 반복 구조가 이전보다 선명해진다.

```text
대형 main.tf
→ 역할별 파일 분할
→ 같은 역할의 반복 블록 식별
→ 후속 count / for_each / for 학습
```

이 흐름은 실제 수업 순서와 코드 구조를 바탕으로 한 해석이다.

### 48-4. 다음 진도에서 볼 최소 기준

```text
1. 어떤 반복 블록을 대상으로 삼는가
2. count와 for_each 중 무엇을 선택하는가
3. index 또는 key가 리소스 주소에 어떻게 반영되는가
4. 기존 직접 참조식이 어떤 Map/List 참조로 바뀌는가
5. plan에서 리소스 주소가 어떻게 달라지는가
```

반복문 문법과 실제 변환 코드는 다음 버전에서 다룬다.
...
```text
[x] 단일 main.tf의 탐색·유지보수 문제 파악
[x] 역할별 .tf 파일 분할 목적 파악
[x] 파일 분할과 Root Module 분리의 차이 정리
[x] 반복문 진도와 연결되는 교육 흐름을 해석으로 구분
```

### 49-2. 코드 구조 확인

```text
[x] 08_quiz의 최상위 HCL 블록 44개 식별
[x] 09_file_division의 최상위 HCL 블록 44개 식별
[x] 블록 종류와 이름의 추가·누락 없음 확인
[x] 각 블록의 정규화된 내용 변경 없음 확인
[x] boot.war, nat_install.tpl, web_install.tpl 동일 확인
[x] 각 블록의 이동 파일 확인
```
...
```text
[ ] 09_file_division terraform init
[ ] 09_file_division terraform validate
[ ] 09_file_division terraform plan
[ ] 실제 AWS apply
```

이번 버전에서는 실행을 요구하지 않았으므로 미완료가 아니라 **범위 제외**로 본다.
...
[ ] for_each 학습
[ ] for expression 학습
[ ] dynamic block 학습 여부 확인
[ ] 반복 적용 전후 State address 변화 확인
[ ] 반복문 적용 코드와 plan 비교
```
...
```text
v9.0은 08_quiz의 종합 구성을 09_file_division에서
역할별 Terraform 파일로 분리한 이유와 결과를 이해한 단계다.

정적 비교 결과, 09는 08의 HCL 블록과 보조 파일 내용을 바꾸지 않고
같은 Root Module 내부에서 위치만 재배치했다.

따라서 이번 단계의 핵심은 인프라 동작 변경이 아니라
코드의 책임 구분과 후속 반복 구조 식별이다.
```
...
```text
v9.0에서는 `08_quiz/main.tf`에 모여 있던 Terraform 구성을 `main.tf`, `networks.tf`, `instances.tf`, `s3.tf`, `load-balancer.tf`, `variables.tf`, `outputs.tf`로 분할하였다. 정적 비교 결과, 08과 09에는 각각 44개의 동일한 최상위 HCL 블록이 존재하며 블록의 종류·이름·정규화된 내용에는 추가, 누락, 변경이 없었다. `boot.war`, `nat_install.tpl`, `web_install.tpl`도 동일하므로, 09는 인프라 동작을 변경한 것이 아니라 같은 Root Module 내부에서 코드를 역할별로 재배치한 것이다. 따라서 State와 Dependency Graph는 하나로 유지되고, 서로 다른 파일의 리소스도 직접 참조할 수 있다. 파일명은 실행 순서를 결정하지 않으며 Terraform은 전체 파일을 결합한 뒤 참조 관계로 생성 순서를 계산한다. 이후 반복문 진도에서는 `networks.tf`에 모인 Subnet과 Route Table Association 같은 반복 구조가 주요 변환 대상이 될 것으로 해석한다.
```
...
## 부록 H. `08_quiz` → `09_file_division` 블록 이동 지도

### H-1. 비교 방법

다음 최상위 HCL 블록을 중괄호 깊이 기준으로 추출해 비교했다.
...
```text
- 블록 종류
- Resource/Data Source 종류
- Terraform 내부 이름
- 블록 본문
- 주석과 공백을 제외한 정규화된 내용
```

보조 파일은 바이트 단위 SHA-256 값으로 비교했다.
...
> [!warning] 검증 범위
> 이 비교는 소스 보존 여부를 확인한 정적 비교다.  
> `terraform validate`, Provider Schema 검증, 실제 AWS API 동작까지 확인한 결과는 아니다.
...
| `08_quiz/main.tf`의 블록 | `09_file_division` 위치 | 개수 | 판정 |
|---|---|---:|---|
| `terraform`, `provider.aws` | `main.tf` | 2 | 위치만 이동 |
| VPC, Subnet, IGW, Route Table, Association, NAT Instance, NAT SG | `networks.tf` | 18 | 위치만 이동 |
| Bastion, Web, RDS, Server SG, DB Subnet Group | `instances.tf` | 7 | 위치만 이동 |
| S3 Bucket/Object, IAM Role/Policy, Instance Profile | `s3.tf` | 5 | 위치만 이동 |
| ALB SG, ALB, Target Group, ACM, Listener, Attachment, Route 53 | `load-balancer.tf` | 9 | 위치만 이동 |
| `variable.username`, `variable.password` | `variables.tf` | 2 | 위치만 이동 |
| `output.public-ip-bastion` | `outputs.tf` | 1 | 위치만 이동 |
| **합계** |  | **44** | **추가·누락·내용 변경 없음** |
...
> networks.tf
> - aws_vpc.quiz-vpc
> - aws_subnet.quiz-public-subnet-2a
> - aws_subnet.quiz-public-subnet-2c
> - aws_subnet.quiz-private-web-subnet-2a
> - aws_subnet.quiz-private-db-subnet-2a
> - aws_subnet.quiz-private-web-subnet-2c
> - aws_subnet.quiz-private-db-subnet-2c
> - aws_internet_gateway.quiz-igw
> - aws_route_table.quiz-public-rt
> - aws_route_table_association.quiz-public-rt-2a
> - aws_route_table_association.quiz-public-rt-2c
> - aws_route_table.quiz-private-rt
> - aws_route_table_association.quiz-private-rt-web-2a
> - aws_route_table_association.quiz-private-rt-db-2a
> - aws_route_table_association.quiz-private-rt-web-2c
> - aws_route_table_association.quiz-private-rt-db-2c
> - aws_instance.quiz-nat-instance
> - aws_security_group.quiz-nat-sg
>
> instances.tf
> - aws_instance.quiz-bastion-instance
> - aws_instance.quiz-web-instance
> - aws_security_group.quiz-bastion-sg
> - aws_security_group.quiz-web-sg
> - aws_security_group.quiz-rds-sg
> - aws_db_instance.quiz-rds-instance
> - aws_db_subnet_group.quiz-rds-subnet-group
>
> s3.tf
> - aws_s3_bucket.quiz-s3-bucket
> - aws_s3_object.quiz-boot-object
> - aws_iam_role.quiz-s3-role
> - aws_iam_role_policy.quiz-s3-role-policy
> - aws_iam_instance_profile.quiz-instance-profile
>
> load-balancer.tf
> - aws_security_group.quiz-alb-sg
> - aws_lb.quiz-alb
> - aws_lb_target_group.quiz-alb-tg
> - data.aws_acm_certificate.quiz-acm
> - aws_lb_listener.quiz-alb-https-listener
> - aws_lb_listener.quiz-alb-http-listener
> - aws_lb_target_group_attachment.quiz-lb-web-attachment
> - data.aws_route53_zone.quiz-route53-zone
> - aws_route53_record.quiz-www
>
> variables.tf
> - variable.username
> - variable.password
