...
> 학습자가 작성한 `stage`의 Networks·Servers 연결은 `terraform validate`를 통과했지만, 강사 답지 전체의 `fmt`, `validate`, `plan`, `apply`와 AWS 동작은 확인하지 않았다.
> v15는 `modules/asg/` 골격과 일부 Input/Output만 작성된 상태다. 현재 세 파일의 정적 내용만 확인했으며, ASG 전체 구성의 `validate`, `plan`, `apply` 성공을 주장하지 않는다.
...
> `modules/asg/main.tf`와 `output.tf`는 아직 비어 있고, Stage/Prod Root Module도 아직 ASG Module을 호출하지 않는다.
...
[ ] private-subnets 등 나머지 Input 계약 확정
...
[ ] Root Module에서 ASG Input으로 전달
[ ] 고정 web-instance-id 경로 제거
```

---

## 63. 세 Checkpoint를 합친 현재 데이터 흐름
...
flowchart LR
    ...
    ASGVAR["ASG variables.tf"]
    ROOT["Stage / Prod main.tf"]

    ELB --> EOUT
    SERVERS --> SOUT
    ROOT -. "아직 기존 v14 연결" .-> ELB
    ASGVAR -. "Input 자리만 선언" .-> ROOT
```

현재는 값을 공개하는 Output과 받을 Input의 일부만 준비됐다. Root Module의 실제 배선과 ASG Resource 구현은 아직 없다.
...
## 64. 다음 구현 순서와 각 단계의 이유

### 64-1. ASG Input 계약 완성
...
이 목록은 현재 Module 소유권을 유지한다는 전제의 **AI 설계안**이다. 실제 코드에 선언하면서 하나씩 소비 여부를 확인한다.

### 64-2. Launch Template 작성

ASG는 EC2를 직접 복제하는 것이 아니라 Launch Template에 정의된 생성 규칙을 사용한다.
...
Launch Template의 `user_data`는 Base64로 인코딩된 값을 요구하므로, 기존 `templatefile()` 결과를 그대로 넣지 않고 Provider 문서의 현재 Schema에 맞춰 작성해야 한다.

### 64-3. Auto Scaling Group 작성
...
v15는 v14의 고정 Web EC2 한 대를 단순히 여러 대로 복제하는 작업이 아니다. EC2 생성 주체를 `Servers` Module에서 새 `ASG` Module로 옮기고, 고정 Instance ID를 ELB에 전달하던 흐름을 ELB의 Target Group ARN을 ASG에 전달하는 흐름으로 뒤집는 구조 전환이다. 현재는 ELB와 Servers가 필요한 값을 Output으로 공개했고 ASG가 일부 Input을 선언한 단계이며, 다음 Checkpoint는 Launch Template와 ASG 본체를 작성해 Root Module에서 네 Module의 Output을 실제로 연결하는 것이다.
