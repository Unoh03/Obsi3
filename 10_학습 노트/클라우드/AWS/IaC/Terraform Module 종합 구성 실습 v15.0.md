...
## 65. 다음 구현 순서와 각 단계의 이유

### 65-1. ASG Input 계약 완성
...
v15는 v14의 고정 Web EC2 한 대를 단순히 여러 대로 복제하는 작업이 아니다. EC2 생성 주체를 `Servers` Module에서 새 `ASG` Module로 옮기고, 고정 Instance ID를 ELB에 전달하던 흐름을 ELB의 Target Group ARN을 ASG에 전달하는 흐름으로 뒤집는 구조 전환이다. 현재 Stage Root에서는 ELB·Servers·Networks·S3 Output을 ASG Input으로 전달하고, Private Subnet Resource Map도 ID List로 변환했다. 다음 Checkpoint는 Launch Template를 작성해 이 입력값을 실제 EC2 생성 규칙으로 소비하는 것이다.
