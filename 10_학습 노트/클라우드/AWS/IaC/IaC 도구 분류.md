...
수업 raw 노트의 핵심 의문은 “Terraform, Ansible, Kubernetes가 각각 뭔지 아직 명확하지 않다”에 가깝다. 아래처럼 먼저 역할을 분리하면 덜 헷갈린다.

| 도구 | 가장 먼저 떠올릴 역할 | 주로 다루는 대상 | 한 줄 감각 |
|---|---|---|---|
| **Terraform** | 프로비저닝 | VPC, Subnet, EC2, Security Group, Load Balancer, DB 같은 인프라 리소스 | “클라우드에 무엇을 만들지” 코드로 선언 |
| **Ansible** | 구성 관리 / 자동화 | 이미 존재하는 서버 안의 패키지, 설정 파일, 서비스 상태 | “서버 안을 어떻게 설정할지” 작업 자동화 |
| **Kubernetes** | 컨테이너 오케스트레이션 | 컨테이너, Pod, Service, Deployment, Replica | “컨테이너 앱을 어떻게 배치하고 계속 살릴지” 관리 |
...
Terraform  = 인프라를 만든다
Ansible    = 서버 안을 설정한다
Kubernetes = 컨테이너 앱을 운영한다
```

이 구분은 절대적인 경계가 아니라 **처음 이해하기 위한 기준선**이다. 실제 현장에서는 세 도구가 함께 쓰일 수 있다.

예시:

```text
Terraform으로 AWS VPC/EC2/EKS 생성
→ Ansible로 서버 패키지와 설정 배포
→ Kubernetes로 컨테이너 애플리케이션 운영
```

## 판단 질문

어떤 도구가 필요한지 헷갈리면 먼저 질문을 바꾼다.

| 질문 | 가까운 도구 |
|---|---|
| AWS에 VPC, Subnet, EC2, ALB를 만들어야 하나? | Terraform |
| 이미 만든 서버에 Nginx, 설정 파일, 계정, 서비스를 맞춰야 하나? | Ansible |
| 컨테이너를 여러 노드에 배치하고 장애 시 자동 복구해야 하나? | Kubernetes |
| 이미지를 만들어 배포 단위를 고정하고 싶은가? | Docker / Packer |
| 같은 작업을 빠르게 한 번 자동화하고 싶은가? | Bash / Python ad-hoc script |
...
Ansible 같은 구성 관리 도구에서 멱등성이 중요한 이유는 “서버 설정 작업을 여러 번 실행해도 최종 상태가 안정적으로 유지되어야 하기 때문”이다.
...
Kubernetes는 “서버를 만드는 도구”라기보다, 이미 준비된 노드 위에서 컨테이너 워크로드를 배치하고 유지하는 쪽에 가깝다.
