...
```text
Private Subnet에 있는 WEB EC2가 Public IP 없이 외부 통신에 성공할 것
```
...
```text
VPC: 192.168.0.0/16
Public Subnet: 192.168.1.0/24, ap-northeast-2a
Private Subnet: 192.168.10.0/24, ap-northeast-2a
NAT Instance private IP: 192.168.1.13
WEB EC2 private IP: 192.168.10.13
```

현재 코드는 2AZ 전체 재현이 아니라, 완료 조건 검증을 우선한 **단일 AZ 축약 구성**이다.

```text
2c AZ, Public Subnet 2, Private Subnet 2, 별도 Bastion Host는 생략했다.
...
## 34. PDF 28p 그림과 현재 코드의 관계

| PDF 28p 요소 | 현재 코드 | 판정 |
|---|---|---|
| VPC `192.168.0.0/16` | `aws_vpc.terra_vpc` | 구현 |
| Internet Gateway | `aws_internet_gateway.gw` | 구현 |
| Public Subnet 1 `192.168.1.0/24` | `terra_open_subnet` | 구현 |
| Private Subnet 1 `192.168.10.0/24` | `terra_close_subnet` | 구현 |
| NAT-GW | `terra_NAT` EC2 | NAT Gateway 대신 NAT Instance로 대체 |
| Private Web EC2 | `terra_WEB` | 구현 |
| Public Route Table | `terra_open_rt` | 구현 |
| Private Route Table | `terra_close_rt` | 구현 |
| Private route `0.0.0.0/0 → NAT` | `network_interface_id = aws_instance.terra_NAT.primary_network_interface_id` | 구현 |
| Public Subnet 2 `192.168.2.0/24` | 없음 | 생략 |
| Private Subnet 2 `192.168.20.0/24` | 없음 | 생략 |
| Bastion Host | `terra_NAT`가 겸함 | 축약 구현 |

판정:

```text
완료 조건 기준:
Private EC2 외부 통신 구조는 구현됨

PDF 그림 완전 재현 기준:
2c AZ와 별도 Bastion Host는 아직 생략됨
```

## 35. 3차 실습에서 발전한 점

3차 실습에서는 PDF 27p 그림을 기준으로 Public/Private Subnet, Route Table, Security Group, EC2 2대를 구성했다. 그러나 그 시점에는 NAT Instance 내부 OS 설정이 빠져 있었다.

이번 4차 실습에서는 다음이 추가되었다.

```text
VPC CIDR을 PDF 28p 기준인 192.168.0.0/16으로 변경
Subnet CIDR을 192.168.1.0/24, 192.168.10.0/24로 변경
AZ를 ap-northeast-2a로 명시
NAT Instance에 고정 private_ip 지정
WEB EC2에 고정 private_ip 지정
NAT Instance user_data에 IP forwarding 설정 추가
NAT Instance user_data에 iptables MASQUERADE 추가
Private Route Table의 기본 경로를 NAT Instance ENI로 지정
```

## 36. NAT Instance 구성 핵심

NAT Instance가 되려면 AWS 리소스 설정과 Linux 내부 설정이 모두 필요하다.

### AWS 쪽 조건

```text
Public Subnet에 NAT Instance 배치
NAT Instance에 Public IP 부여
NAT Instance의 source_dest_check = false
Private Route Table의 0.0.0.0/0을 NAT Instance ENI로 지정
Private Subnet과 Private Route Table을 association
```
...
```text
IP forwarding 활성화
iptables MASQUERADE 규칙 추가
iptables 규칙 저장
iptables service 활성화
```

현재 `user_data`에서 해당하는 부분:
...
## 37. VS Code 자동완성 `user_data`와의 차이

VS Code 자동완성은 다음과 비슷한 코드를 제안했다.
...
이 코드는 NAT Instance용으로는 부족할 수 있다.

| 항목 | 자동완성 코드 | 현재 코드 |
|---|---|---|
| IP forwarding | 없음 | `sysctl -w net.ipv4.ip_forward=1` |
| forwarding 영구화 | 없음 | `/etc/sysctl.d/99-nat.conf` |
| outbound interface | `eth0` 고정 | `ip route`로 자동 탐지 |
| MASQUERADE | 있음 | 있음 |
| iptables 저장 | 있음 | 있음 |
| 실습 속도 | `yum update -y`로 느릴 수 있음 | 필수 패키지만 설치 |

핵심 차이:

```text
iptables MASQUERADE는 주소 변환이고,
ip_forward는 패킷 전달 허용이다.
NAT Instance에는 둘 다 필요하다.
```

## 38. 시행착오와 에러 메시지

### 38-1. NAT Gateway와 NAT Instance 혼동

초기 해석에서는 PDF 28p 그림의 `NAT-GW`를 그대로 따라 NAT Gateway로 구현하는 방향을 검토했다. 그러나 NAT Gateway는 비용이 발생하므로 수업 실습에서는 NAT Instance로 대체하는 것이 적절하다고 판단했다.
...
완료 조건:
Private EC2에서 외부통신 성공
```

### 38-2. AWS 예약 IP 사용 오류

처음에는 EC2 private IP를 다음처럼 지정했다.
...
`terraform apply` 시 다음 오류가 발생했다.

```text
Error: creating EC2 Instance: operation error EC2: RunInstances, https response error StatusCode: 400, RequestID: c492f4d8-af73-439a-b9d2-d68eef0df796, api error InvalidParameterValue: Address 192.168.10.1 is in subnet's reserved address range

  with aws_instance.terra_WEB,
  on main.tf line 98, in resource "aws_instance" "terra_WEB":
  98: resource "aws_instance" "terra_WEB":
```

```text
Error: creating EC2 Instance: operation error EC2: RunInstances, https response error StatusCode: 400, RequestID: 02e8c358-1b52-4f2a-b99c-75120500c699, api error InvalidParameterValue: Address 192.168.1.1 is in subnet's reserved address range

  with aws_instance.terra_NAT,
  on main.tf line 128, in resource "aws_instance" "terra_NAT":
  128: resource "aws_instance" "terra_NAT":
```

원인:

```text
AWS는 각 Subnet CIDR에서 일부 IP를 예약한다.
`.1`은 VPC router 주소로 예약되므로 EC2 private IP로 사용할 수 없다.
```

조치:

```hcl
terra_WEB private_ip = "192.168.10.13"
terra_NAT private_ip = "192.168.1.13"
```

수정 후 `terraform apply`가 성공했다.

### 38-3. 2c AZ 생략 판단

PDF 그림에는 2c AZ가 포함되어 있으나, 완료 조건이 “Private에서 외부통신”이라면 2c AZ는 필수 조건이 아니다.

현재 판단:

```text
Private EC2 outbound 통신 검증:
1AZ 구성으로 충분

PDF 그림 완전 재현:
2c AZ, Public Subnet 2, Private Subnet 2, Bastion Host 분리 필요
```

따라서 이번 중간 저장본은 2c를 생략하고, 단일 AZ에서 NAT Instance 기반 outbound 통신을 먼저 검증하는 버전이다.

### 38-4. Bastion과 NAT 역할 분리 여부

운영 관점에서는 Bastion Host와 NAT Instance는 역할이 다르다.

```text
Bastion Host:
외부 관리자가 Private EC2로 들어가는 SSH 경유지

NAT Instance:
Private EC2가 외부 인터넷으로 나가는 outbound 변환 장치
```

현재 실습에서는 비용과 단순화를 위해 `terra_NAT`가 두 역할을 겸한다.
...
### 39-1. NAT Instance 접속

로컬 PC에서 NAT Instance로 접속한다.

```powershell
ssh -i .\asd-open.pem ec2-user@<NAT_PUBLIC_IP>
```

### 39-2. WEB EC2 접속용 키 복사

`terra_WEB`은 `key_name = "asd-close"`를 사용하므로, NAT Instance 안에서 WEB으로 들어가려면 `asd-close.pem`이 필요하다.

로컬 PC에서 실행:

```powershell
scp -i .\asd-open.pem .\asd-close.pem ec2-user@<NAT_PUBLIC_IP>:~/asd-close.pem
```

NAT Instance 안에서 권한 수정:
...
```bash
ssh -i ~/asd-close.pem ec2-user@192.168.10.13
```

### 39-3. NAT Instance 내부 설정 확인
...
```text
POSTROUTING chain에 MASQUERADE 규칙 존재
```

### 39-4. Private WEB EC2에서 외부통신 확인

Private WEB EC2에서 실행:
...
```text
전 세계에서 모든 프로토콜과 모든 포트 허용
```

실습 중 임시 설정으로는 가능하지만, 보안적으로는 넓다.

개선 방향:

```text
Bastion 접속:
내 공인 IP/32에서 TCP 22만 허용

NAT 기능:
Private Subnet CIDR에서 오는 트래픽 허용
...
```text
Public Subnet 2: 192.168.2.0/24, ap-northeast-2c
Private Subnet 2: 192.168.20.0/24, ap-northeast-2c
별도 Bastion Host
