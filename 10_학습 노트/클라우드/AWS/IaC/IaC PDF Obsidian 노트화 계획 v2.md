---
title: IaC PDF Obsidian 노트화 계획 v2
created: 2026-07-06
updated: 2026-07-06
status: active
type: workflow-note
...
| 계획 노트 작성 | 완료 |
| `Iac.pdf` 정독 | 완료 |
| Source Digest 1차본 | 완료: `IaC - Source Digest.md` |
| Source Digest 2차본 | 완료: `IaC - Source Digest v2.md` |
| 페이지 이미지 보존 | 완료: `assets/Iac_page_01.png` ~ `assets/Iac_page_52.png` |
| Coverage Map | 완료: p.1~p.52 |
| 이미지 속 코드/터미널/도식 보강 | 2차 보강 완료 |
| 공식 문서 대조 | 미완료 |
| Concept Note 작성 | 미완료 |
| Lab Note 작성 | 미완료 |
| MOC 작성 | 미완료 |
...
- `IaC - Source Digest.md`는 1차본으로 보관한다.
- `IaC - Source Digest v2.md`를 이후 Concept Note 작성의 기준본으로 사용한다.
- PDF 원문 판단이 필요하면 `assets/Iac_page_XX.png`를 최종 기준으로 본다.
- v2 Appendix의 코드 전사본은 검색성과 노트화를 위한 보강본이며, 애매한 문자는 원본 이미지와 대조한다.
- `IaC_Source_Digest_v2_FIXED_package.zip`는 계획 노트까지 현재 상태로 갱신한 패키지다.
...
| 기준 Source Digest | `IaC - Source Digest v2.md` |
| 보관본 | `IaC - Source Digest.md` |
| 권장 패키지 | `IaC_Source_Digest_v2_FIXED_package.zip` |
...
Concept Note 작성 전에 다음 항목은 Terraform/AWS 공식 문서로 확인한다.

- `terraform destory` 표기: 실제 명령어는 `terraform destroy`인지 확인
- `.terraform.lock.hcl`의 역할
- `count`, `for_each`와 Module 사용 제약
- `terraform_remote_state` 사용 방식
- Backend S3 + DynamoDB Lock 구성 방식
- 외부 Module 버전 `terraform-aws-modules/vpc/aws`, `terraform-aws-modules/security-group/aws`의 현재 호환성
- p.35의 Output 이름 불일치: `EC2_Pub_IP` vs `public_ip`
- 강의자료 코드가 현재 Terraform AWS Provider 버전에서도 그대로 동작하는지 확인
...
우선 `IaC - Source Digest v2.md`를 기준본으로 사용하고, Concept Note 작성 전에 확인 필요 항목을 공식 문서로 대조하라.
```

---

## Definition of Done - 현재 단계

Source Digest 단계는 다음 조건을 만족했으므로 완료로 본다.

- [x] `IaC - Source Digest v2.md` 생성
- [x] PDF 1~52쪽 Coverage Map 반영
- [x] 페이지 이미지 52장 보존
- [x] 주요 코드 페이지 전사 보강
- [x] 터미널 출력 페이지 전사 보강
- [x] 아키텍처 도식 구조 트리 보강
