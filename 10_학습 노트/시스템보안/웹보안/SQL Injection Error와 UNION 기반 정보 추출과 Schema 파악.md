...
이 노트는 p.122-131의 정보 추출 흐름을 다룬다. p.132의 방어책은 별도 `SQL Injection 방어` 노트에서 정리한다.
...
[[SQL Injection 인증 우회 실습|SQL Injection 인증 우회]]는 보통 로그인 성공 여부를 바꾼다.
...
실제 `updatexml()`, `concat()` 조합과 관찰 결과는 [[SQL Injection Error 기반 DB명 정보 추출 실습|SQL Injection Error 기반 정보 추출 실습]]에 정리했다.
...
이 단계에서 실제 `LIMIT`, `group_concat()`을 어떻게 써 보았고 어디서 잘렸는지는 [[SQL Injection Error 기반 DB명 정보 추출 실습|SQL Injection Error 기반 정보 추출 실습]]에 남긴다.
...
- p.132 방어책은 이 노트의 범위가 아니다. 방어는 별도 노트에서 공식 자료와 함께 정리한다.
...
- [[SQL Injection Error 기반 DB명 정보 추출 실습|SQL Injection Error 기반 정보 추출 실습]]
